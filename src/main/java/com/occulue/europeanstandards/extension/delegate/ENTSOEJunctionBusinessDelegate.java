 ...
 *   <li>Improving the available of ENTSOEJunction related services in the case of a ENTSOEJunction
 *       business related service failing.
 *   <li>Exposes a simpler, uniform ENTSOEJunction interface to the business tier, making it easy
 *       for clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill ENTSOEJunction business
 *       related services.
 * </ol>
 *
 * <p>
 ...
 */
public class ENTSOEJunctionBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createENTSOEJunction(CreateENTSOEJunctionCommand command)
    ...
    try {
      ...
      // --------------------------------------
      if (command.getENTSOEJunctionId() == null) command.setENTSOEJunctionId(UUID.randomUUID());
      ...
      // ---------------------------------------
      // issue the CreateENTSOEJunctionCommand - by convention the future return value for a create
      // command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateENTSOEJunctionCommand of ENTSOEJunction is "
              + command);
  ...
  public CompletableFuture<Void> delete(DeleteENTSOEJunctionCommand command)
    ...
    try {
      ...
      final String errMsg =
          "Unable to delete ENTSOEJunction using Id = " + command.getENTSOEJunctionId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
  ...
  public ENTSOEJunction getENTSOEJunction(ENTSOEJunctionFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<ENTSOEJunction> futureEntity =
          queryGateway.query(
              new FindENTSOEJunctionQuery(
                  new LoadENTSOEJunctionFilter(summary.getENTSOEJunctionId())),
              ResponseTypes.instanceOf(ENTSOEJunction.class));

      entity = futureEntity.get();
    } catch (Exception exc) {
      final String errMsg =
          "Unable to locate ENTSOEJunction with id " + summary.getENTSOEJunctionId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
   ...
   */
  public List<ENTSOEJunction> getAllENTSOEJunction() throws ProcessingException {
    List<ENTSOEJunction> list = null;

    try {
      CompletableFuture<List<ENTSOEJunction>> futureList =
          queryGateway.query(
   ...
   */
  private ENTSOEJunction load(UUID id) throws ProcessingException {
    eNTSOEJunction =
        ENTSOEJunctionBusinessDelegate.getENTSOEJunctionInstance()
            .getENTSOEJunction(new ENTSOEJunctionFetchOneSummary(id));
    return eNTSOEJunction;
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(ENTSOEJunctionBusinessDelegate.class.getName());
}
