 ...
 *   <li>Improving the available of LoadDynamics related services in the case of a LoadDynamics
 *       business related service failing.
 *   <li>Exposes a simpler, uniform LoadDynamics interface to the business tier, making it easy for
 *       clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill LoadDynamics business
 *       related services.
 * </ol>
 *
 * <p>
 ...
 */
public class LoadDynamicsBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createLoadDynamics(CreateLoadDynamicsCommand command)
    ...
    try {
      ...
      // --------------------------------------
      if (command.getLoadDynamicsId() == null) command.setLoadDynamicsId(UUID.randomUUID());
      ...
      // ---------------------------------------
      // issue the CreateLoadDynamicsCommand - by convention the future return value for a create
      // command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateLoadDynamicsCommand of LoadDynamics is "
              + command);
  ...
  public CompletableFuture<Void> delete(DeleteLoadDynamicsCommand command)
    ...
    try {
      ...
      final String errMsg =
          "Unable to delete LoadDynamics using Id = " + command.getLoadDynamicsId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
  ...
  public LoadDynamics getLoadDynamics(LoadDynamicsFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<LoadDynamics> futureEntity =
          queryGateway.query(
              new FindLoadDynamicsQuery(new LoadLoadDynamicsFilter(summary.getLoadDynamicsId())),
   ...
   */
  public List<LoadDynamics> getAllLoadDynamics() throws ProcessingException {
    List<LoadDynamics> list = null;

    try {
      CompletableFuture<List<LoadDynamics>> futureList =
          queryGateway.query(
   ...
   * @return LoadDynamics
   */
  private LoadDynamics load(UUID id) throws ProcessingException {
    loadDynamics =
        LoadDynamicsBusinessDelegate.getLoadDynamicsInstance()
            .getLoadDynamics(new LoadDynamicsFetchOneSummary(id));
    return loadDynamics;
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(LoadDynamicsBusinessDelegate.class.getName());
}
