 ...
 *   <li>Improving the available of WindAeroConstIEC related services in the case of a
 *       WindAeroConstIEC business related service failing.
 *   <li>Exposes a simpler, uniform WindAeroConstIEC interface to the business tier, making it easy
 *       for clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill WindAeroConstIEC business
 *       related services.
 * </ol>
 *
 * <p>
 ...
 */
public class WindAeroConstIECBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createWindAeroConstIEC(CreateWindAeroConstIECCommand command)
    ...
    try {
      ...
      // --------------------------------------
      if (command.getWindAeroConstIECId() == null) command.setWindAeroConstIECId(UUID.randomUUID());
      ...
      // ---------------------------------------
      // issue the CreateWindAeroConstIECCommand - by convention the future return value for a
      // create command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateWindAeroConstIECCommand of WindAeroConstIEC is "
              + command);
  ...
  public CompletableFuture<Void> delete(DeleteWindAeroConstIECCommand command)
    ...
    try {
      ...
      final String errMsg =
          "Unable to delete WindAeroConstIEC using Id = " + command.getWindAeroConstIECId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
  ...
  public WindAeroConstIEC getWindAeroConstIEC(WindAeroConstIECFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<WindAeroConstIEC> futureEntity =
          queryGateway.query(
              new FindWindAeroConstIECQuery(
                  new LoadWindAeroConstIECFilter(summary.getWindAeroConstIECId())),
              ResponseTypes.instanceOf(WindAeroConstIEC.class));

      entity = futureEntity.get();
    } catch (Exception exc) {
      final String errMsg =
          "Unable to locate WindAeroConstIEC with id " + summary.getWindAeroConstIECId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
   ...
   */
  public List<WindAeroConstIEC> getAllWindAeroConstIEC() throws ProcessingException {
    List<WindAeroConstIEC> list = null;

    try {
      CompletableFuture<List<WindAeroConstIEC>> futureList =
          queryGateway.query(
              new FindAllWindAeroConstIECQuery(),
   ...
   */
  private WindAeroConstIEC load(UUID id) throws ProcessingException {
    windAeroConstIEC =
        WindAeroConstIECBusinessDelegate.getWindAeroConstIECInstance()
            .getWindAeroConstIEC(new WindAeroConstIECFetchOneSummary(id));
    return windAeroConstIEC;
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(WindAeroConstIECBusinessDelegate.class.getName());
}
