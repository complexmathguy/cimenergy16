 ...
 *   <li>Improving the available of Unresolvedname related services in the case of a Unresolvedname
 *       business related service failing.
 *   <li>Exposes a simpler, uniform Unresolvedname interface to the business tier, making it easy
 *       for clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill Unresolvedname business
 *       related services.
 * </ol>
 *
 * <p>
 ...
 */
public class UnresolvednameBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createUnresolvedname(CreateUnresolvednameCommand command)
    ...
    try {
      ...
      // --------------------------------------
      if (command.getUnresolvednameId() == null) command.setUnresolvednameId(UUID.randomUUID());
      ...
      // ---------------------------------------
      // issue the CreateUnresolvednameCommand - by convention the future return value for a create
      // command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateUnresolvednameCommand of Unresolvedname is "
              + command);
  ...
  public CompletableFuture<Void> delete(DeleteUnresolvednameCommand command)
    ...
    try {
      ...
      final String errMsg =
          "Unable to delete Unresolvedname using Id = " + command.getUnresolvednameId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
  ...
  public Unresolvedname getUnresolvedname(UnresolvednameFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<Unresolvedname> futureEntity =
          queryGateway.query(
              new FindUnresolvednameQuery(
                  new LoadUnresolvednameFilter(summary.getUnresolvednameId())),
              ResponseTypes.instanceOf(Unresolvedname.class));

      entity = futureEntity.get();
    } catch (Exception exc) {
      final String errMsg =
          "Unable to locate Unresolvedname with id " + summary.getUnresolvednameId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
   ...
   */
  public List<Unresolvedname> getAllUnresolvedname() throws ProcessingException {
    List<Unresolvedname> list = null;

    try {
      CompletableFuture<List<Unresolvedname>> futureList =
          queryGateway.query(
   ...
   */
  private Unresolvedname load(UUID id) throws ProcessingException {
    unresolvedname =
        UnresolvednameBusinessDelegate.getUnresolvednameInstance()
            .getUnresolvedname(new UnresolvednameFetchOneSummary(id));
    return unresolvedname;
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(UnresolvednameBusinessDelegate.class.getName());
}
