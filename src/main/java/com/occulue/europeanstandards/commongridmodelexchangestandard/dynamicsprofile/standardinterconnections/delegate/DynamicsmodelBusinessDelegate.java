 ...
 *   <li>Improving the available of Dynamicsmodel related services in the case of a Dynamicsmodel
 *       business related service failing.
 *   <li>Exposes a simpler, uniform Dynamicsmodel interface to the business tier, making it easy for
 *       clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill Dynamicsmodel business
 *       related services.
 * </ol>
 *
 * <p>
 ...
 */
public class DynamicsmodelBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createDynamicsmodel(CreateDynamicsmodelCommand command)
    ...
    try {
      ...
      // --------------------------------------
      if (command.getDynamicsmodelId() == null) command.setDynamicsmodelId(UUID.randomUUID());
      ...
      // ---------------------------------------
      // issue the CreateDynamicsmodelCommand - by convention the future return value for a create
      // command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateDynamicsmodelCommand of Dynamicsmodel is "
              + command);
  ...
  public CompletableFuture<Void> delete(DeleteDynamicsmodelCommand command)
    ...
    try {
      ...
      final String errMsg =
          "Unable to delete Dynamicsmodel using Id = " + command.getDynamicsmodelId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
  ...
  public Dynamicsmodel getDynamicsmodel(DynamicsmodelFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<Dynamicsmodel> futureEntity =
          queryGateway.query(
              new FindDynamicsmodelQuery(new LoadDynamicsmodelFilter(summary.getDynamicsmodelId())),
              ResponseTypes.instanceOf(Dynamicsmodel.class));

      entity = futureEntity.get();
    } catch (Exception exc) {
      final String errMsg =
          "Unable to locate Dynamicsmodel with id " + summary.getDynamicsmodelId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
   ...
   */
  public List<Dynamicsmodel> getAllDynamicsmodel() throws ProcessingException {
    List<Dynamicsmodel> list = null;

    try {
      CompletableFuture<List<Dynamicsmodel>> futureList =
          queryGateway.query(
   ...
   */
  private Dynamicsmodel load(UUID id) throws ProcessingException {
    dynamicsmodel =
        DynamicsmodelBusinessDelegate.getDynamicsmodelInstance()
            .getDynamicsmodel(new DynamicsmodelFetchOneSummary(id));
    return dynamicsmodel;
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(DynamicsmodelBusinessDelegate.class.getName());
}
