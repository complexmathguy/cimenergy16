 ...
 *   <li>Improving the available of DiagramObjectGluePoint related services in the case of a
 *       DiagramObjectGluePoint business related service failing.
 *   <li>Exposes a simpler, uniform DiagramObjectGluePoint interface to the business tier, making it
 *       easy for clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill DiagramObjectGluePoint
 ...
 */
public class DiagramObjectGluePointBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
   ...
   */
  public CompletableFuture<UUID> createDiagramObjectGluePoint(
      CreateDiagramObjectGluePointCommand command)
    ...
    try {
      ...
      // ---------------------------------------
      // issue the CreateDiagramObjectGluePointCommand - by convention the future return value for a
      // create command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateDiagramObjectGluePointCommand of DiagramObjectGluePoint is "
              + command);
   ...
   * @exception IllegalArgumentException
   */
  public CompletableFuture<Void> updateDiagramObjectGluePoint(
      UpdateDiagramObjectGluePointCommand command)
      throws ProcessingException, IllegalArgumentException {
  ...
  public CompletableFuture<Void> delete(DeleteDiagramObjectGluePointCommand command)
    ...
    try {
      ...
      final String errMsg =
          "Unable to delete DiagramObjectGluePoint using Id = "
  ...
  public DiagramObjectGluePoint getDiagramObjectGluePoint(
      DiagramObjectGluePointFetchOneSummary summary)
      throws ProcessingException, IllegalArgumentException {

    if (summary == null)
      throw new IllegalArgumentException(
          "DiagramObjectGluePointFetchOneSummary arg cannot be null");
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<DiagramObjectGluePoint> futureEntity =
          queryGateway.query(
              new FindDiagramObjectGluePointQuery(
                  new LoadDiagramObjectGluePointFilter(summary.getDiagramObjectGluePointId())),
              ResponseTypes.instanceOf(DiagramObjectGluePoint.class));

      entity = futureEntity.get();
    } catch (Exception exc) {
      final String errMsg =
          "Unable to locate DiagramObjectGluePoint with id "
   ...
   */
  public List<DiagramObjectGluePoint> getAllDiagramObjectGluePoint() throws ProcessingException {
    List<DiagramObjectGluePoint> list = null;

    try {
      CompletableFuture<List<DiagramObjectGluePoint>> futureList =
          queryGateway.query(
   ...
   */
  private DiagramObjectGluePoint load(UUID id) throws ProcessingException {
    diagramObjectGluePoint =
        DiagramObjectGluePointBusinessDelegate.getDiagramObjectGluePointInstance()
            .getDiagramObjectGluePoint(new DiagramObjectGluePointFetchOneSummary(id));
    return diagramObjectGluePoint;
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(DiagramObjectGluePointBusinessDelegate.class.getName());
}
