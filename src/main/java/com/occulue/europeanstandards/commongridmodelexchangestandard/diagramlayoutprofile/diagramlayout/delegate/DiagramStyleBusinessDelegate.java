 ...
 *   <li>Improving the available of DiagramStyle related services in the case of a DiagramStyle
 *       business related service failing.
 *   <li>Exposes a simpler, uniform DiagramStyle interface to the business tier, making it easy for
 *       clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill DiagramStyle business
 *       related services.
 * </ol>
 *
 * <p>
 ...
 */
public class DiagramStyleBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createDiagramStyle(CreateDiagramStyleCommand command)
    ...
    try {
      ...
      // --------------------------------------
      if (command.getDiagramStyleId() == null) command.setDiagramStyleId(UUID.randomUUID());
      ...
      // ---------------------------------------
      // issue the CreateDiagramStyleCommand - by convention the future return value for a create
      // command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateDiagramStyleCommand of DiagramStyle is "
              + command);
  ...
  public CompletableFuture<Void> delete(DeleteDiagramStyleCommand command)
    ...
    try {
      ...
      final String errMsg =
          "Unable to delete DiagramStyle using Id = " + command.getDiagramStyleId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
  ...
  public DiagramStyle getDiagramStyle(DiagramStyleFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<DiagramStyle> futureEntity =
          queryGateway.query(
              new FindDiagramStyleQuery(new LoadDiagramStyleFilter(summary.getDiagramStyleId())),
   ...
   */
  public List<DiagramStyle> getAllDiagramStyle() throws ProcessingException {
    List<DiagramStyle> list = null;

    try {
      CompletableFuture<List<DiagramStyle>> futureList =
          queryGateway.query(
   ...
   * @return DiagramStyle
   */
  private DiagramStyle load(UUID id) throws ProcessingException {
    diagramStyle =
        DiagramStyleBusinessDelegate.getDiagramStyleInstance()
            .getDiagramStyle(new DiagramStyleFetchOneSummary(id));
    return diagramStyle;
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(DiagramStyleBusinessDelegate.class.getName());
}
