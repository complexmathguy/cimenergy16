 ...
 *   <li>Improving the available of GeographicalRegion related services in the case of a
 *       GeographicalRegion business related service failing.
 *   <li>Exposes a simpler, uniform GeographicalRegion interface to the business tier, making it
 *       easy for clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill GeographicalRegion
 ...
 */
public class GeographicalRegionBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createGeographicalRegion(CreateGeographicalRegionCommand command)
    ...
    try {
      ...
      // ---------------------------------------
      // issue the CreateGeographicalRegionCommand - by convention the future return value for a
      // create command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateGeographicalRegionCommand of GeographicalRegion is "
              + command);
  ...
  public CompletableFuture<Void> delete(DeleteGeographicalRegionCommand command)
    ...
    try {
      ...
      final String errMsg =
          "Unable to delete GeographicalRegion using Id = " + command.getGeographicalRegionId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
  ...
  public GeographicalRegion getGeographicalRegion(GeographicalRegionFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<GeographicalRegion> futureEntity =
          queryGateway.query(
              new FindGeographicalRegionQuery(
                  new LoadGeographicalRegionFilter(summary.getGeographicalRegionId())),
              ResponseTypes.instanceOf(GeographicalRegion.class));

      entity = futureEntity.get();
    } catch (Exception exc) {
      final String errMsg =
          "Unable to locate GeographicalRegion with id " + summary.getGeographicalRegionId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
   ...
   */
  public List<GeographicalRegion> getAllGeographicalRegion() throws ProcessingException {
    List<GeographicalRegion> list = null;

    try {
      CompletableFuture<List<GeographicalRegion>> futureList =
          queryGateway.query(
   ...
   */
  private GeographicalRegion load(UUID id) throws ProcessingException {
    geographicalRegion =
        GeographicalRegionBusinessDelegate.getGeographicalRegionInstance()
            .getGeographicalRegion(new GeographicalRegionFetchOneSummary(id));
    return geographicalRegion;
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(GeographicalRegionBusinessDelegate.class.getName());
}
