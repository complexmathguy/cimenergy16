...
import com.occulue.europeanstandards.commongridmodelexchangestandard.equipmentboundaryprofile.wires.validator.*;
 ...
 *   <li>Improving the available of Connector related services in the case of a Connector business
 *       related service failing.
 *   <li>Exposes a simpler, uniform Connector interface to the business tier, making it easy for
 *       clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill Connector business related
 *       services.
 * </ol>
 *
 * <p>
 ...
 */
public class ConnectorBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createConnector(CreateConnectorCommand command)
    ...
    try {
      ...
      // --------------------------------------
      if (command.getConnectorId() == null) command.setConnectorId(UUID.randomUUID());
      ...
      // ---------------------------------------
      // issue the CreateConnectorCommand - by convention the future return value for a create
      // command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateConnectorCommand of Connector is " + command);
  ...
  public Connector getConnector(ConnectorFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<Connector> futureEntity =
          queryGateway.query(
              new FindConnectorQuery(new LoadConnectorFilter(summary.getConnectorId())),
   ...
   */
  public List<Connector> getAllConnector() throws ProcessingException {
    List<Connector> list = null;

    try {
      CompletableFuture<List<Connector>> futureList =
          queryGateway.query(
              new FindAllConnectorQuery(), ResponseTypes.multipleInstancesOf(Connector.class));
   ...
   * @return Connector
   */
  private Connector load(UUID id) throws ProcessingException {
    connector =
        ConnectorBusinessDelegate.getConnectorInstance()
            .getConnector(new ConnectorFetchOneSummary(id));
    return connector;
  }
