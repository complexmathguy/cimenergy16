 ...
 *   <li>Improving the available of ConductingEquipment related services in the case of a
 *       ConductingEquipment business related service failing.
 *   <li>Exposes a simpler, uniform ConductingEquipment interface to the business tier, making it
 *       easy for clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill ConductingEquipment
 ...
 */
public class ConductingEquipmentBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createConductingEquipment(CreateConductingEquipmentCommand command)
    ...
    try {
      ...
      // ---------------------------------------
      // issue the CreateConductingEquipmentCommand - by convention the future return value for a
      // create command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateConductingEquipmentCommand of ConductingEquipment is "
              + command);
  ...
  public CompletableFuture<Void> delete(DeleteConductingEquipmentCommand command)
    ...
    try {
      ...
      final String errMsg =
          "Unable to delete ConductingEquipment using Id = " + command.getConductingEquipmentId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
  ...
  public ConductingEquipment getConductingEquipment(ConductingEquipmentFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<ConductingEquipment> futureEntity =
          queryGateway.query(
              new FindConductingEquipmentQuery(
                  new LoadConductingEquipmentFilter(summary.getConductingEquipmentId())),
              ResponseTypes.instanceOf(ConductingEquipment.class));

      entity = futureEntity.get();
    } catch (Exception exc) {
      final String errMsg =
          "Unable to locate ConductingEquipment with id " + summary.getConductingEquipmentId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
   ...
   */
  public List<ConductingEquipment> getAllConductingEquipment() throws ProcessingException {
    List<ConductingEquipment> list = null;

    try {
      CompletableFuture<List<ConductingEquipment>> futureList =
          queryGateway.query(
   ...
   */
  private ConductingEquipment load(UUID id) throws ProcessingException {
    conductingEquipment =
        ConductingEquipmentBusinessDelegate.getConductingEquipmentInstance()
            .getConductingEquipment(new ConductingEquipmentFetchOneSummary(id));
    return conductingEquipment;
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(ConductingEquipmentBusinessDelegate.class.getName());
}
