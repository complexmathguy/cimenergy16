 ...
 *   <li>Improving the available of EquipmentContainer related services in the case of a
 *       EquipmentContainer business related service failing.
 *   <li>Exposes a simpler, uniform EquipmentContainer interface to the business tier, making it
 *       easy for clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill EquipmentContainer
 ...
 */
public class EquipmentContainerBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createEquipmentContainer(CreateEquipmentContainerCommand command)
    ...
    try {
      ...
      // ---------------------------------------
      // issue the CreateEquipmentContainerCommand - by convention the future return value for a
      // create command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateEquipmentContainerCommand of EquipmentContainer is "
              + command);
  ...
  public CompletableFuture<Void> delete(DeleteEquipmentContainerCommand command)
    ...
    try {
      ...
      final String errMsg =
          "Unable to delete EquipmentContainer using Id = " + command.getEquipmentContainerId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
  ...
  public EquipmentContainer getEquipmentContainer(EquipmentContainerFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<EquipmentContainer> futureEntity =
          queryGateway.query(
              new FindEquipmentContainerQuery(
                  new LoadEquipmentContainerFilter(summary.getEquipmentContainerId())),
              ResponseTypes.instanceOf(EquipmentContainer.class));

      entity = futureEntity.get();
    } catch (Exception exc) {
      final String errMsg =
          "Unable to locate EquipmentContainer with id " + summary.getEquipmentContainerId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
   ...
   */
  public List<EquipmentContainer> getAllEquipmentContainer() throws ProcessingException {
    List<EquipmentContainer> list = null;

    try {
      CompletableFuture<List<EquipmentContainer>> futureList =
          queryGateway.query(
   ...
   */
  private EquipmentContainer load(UUID id) throws ProcessingException {
    equipmentContainer =
        EquipmentContainerBusinessDelegate.getEquipmentContainerInstance()
            .getEquipmentContainer(new EquipmentContainerFetchOneSummary(id));
    return equipmentContainer;
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(EquipmentContainerBusinessDelegate.class.getName());
}
