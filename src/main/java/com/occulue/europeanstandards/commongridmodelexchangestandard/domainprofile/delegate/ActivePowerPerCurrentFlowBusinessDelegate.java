 ...
 *   <li>Improving the available of ActivePowerPerCurrentFlow related services in the case of a
 *       ActivePowerPerCurrentFlow business related service failing.
 *   <li>Exposes a simpler, uniform ActivePowerPerCurrentFlow interface to the business tier, making
 *       it easy for clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill ActivePowerPerCurrentFlow
 ...
 */
public class ActivePowerPerCurrentFlowBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
   ...
   */
  public CompletableFuture<UUID> createActivePowerPerCurrentFlow(
      CreateActivePowerPerCurrentFlowCommand command)
    ...
    try {
      ...
      // ---------------------------------------
      // issue the CreateActivePowerPerCurrentFlowCommand - by convention the future return value
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateActivePowerPerCurrentFlowCommand of ActivePowerPerCurrentFlow is "
              + command);
   ...
   * @exception IllegalArgumentException
   */
  public CompletableFuture<Void> updateActivePowerPerCurrentFlow(
      UpdateActivePowerPerCurrentFlowCommand command)
      throws ProcessingException, IllegalArgumentException {
  ...
  public CompletableFuture<Void> delete(DeleteActivePowerPerCurrentFlowCommand command)
    ...
    try {
      ...
      final String errMsg =
          "Unable to delete ActivePowerPerCurrentFlow using Id = "
              + command.getActivePowerPerCurrentFlowId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
   ...
   * @exception IllegalArgumentException
   */
  public ActivePowerPerCurrentFlow getActivePowerPerCurrentFlow(
      ActivePowerPerCurrentFlowFetchOneSummary summary)
      throws ProcessingException, IllegalArgumentException {

    if (summary == null)
      throw new IllegalArgumentException(
          "ActivePowerPerCurrentFlowFetchOneSummary arg cannot be null");
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<ActivePowerPerCurrentFlow> futureEntity =
          queryGateway.query(
              new FindActivePowerPerCurrentFlowQuery(
                  new LoadActivePowerPerCurrentFlowFilter(
      ...
      final String errMsg =
          "Unable to locate ActivePowerPerCurrentFlow with id "
              + summary.getActivePowerPerCurrentFlowId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
  ...
  public List<ActivePowerPerCurrentFlow> getAllActivePowerPerCurrentFlow()
    ...
    try {
      CompletableFuture<List<ActivePowerPerCurrentFlow>> futureList =
          queryGateway.query(
   ...
   */
  private ActivePowerPerCurrentFlow load(UUID id) throws ProcessingException {
    activePowerPerCurrentFlow =
        ActivePowerPerCurrentFlowBusinessDelegate.getActivePowerPerCurrentFlowInstance()
            .getActivePowerPerCurrentFlow(new ActivePowerPerCurrentFlowFetchOneSummary(id));
    return activePowerPerCurrentFlow;
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(ActivePowerPerCurrentFlowBusinessDelegate.class.getName());
}
