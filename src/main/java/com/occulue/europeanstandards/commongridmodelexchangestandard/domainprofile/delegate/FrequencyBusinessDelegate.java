 ...
 *   <li>Improving the available of Frequency related services in the case of a Frequency business
 *       related service failing.
 *   <li>Exposes a simpler, uniform Frequency interface to the business tier, making it easy for
 *       clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill Frequency business related
 *       services.
 * </ol>
 *
 * <p>
 ...
 */
public class FrequencyBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createFrequency(CreateFrequencyCommand command)
    ...
    try {
      ...
      // --------------------------------------
      if (command.getFrequencyId() == null) command.setFrequencyId(UUID.randomUUID());
      ...
      // ---------------------------------------
      // issue the CreateFrequencyCommand - by convention the future return value for a create
      // command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateFrequencyCommand of Frequency is " + command);
  ...
  public Frequency getFrequency(FrequencyFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<Frequency> futureEntity =
          queryGateway.query(
              new FindFrequencyQuery(new LoadFrequencyFilter(summary.getFrequencyId())),
   ...
   */
  public List<Frequency> getAllFrequency() throws ProcessingException {
    List<Frequency> list = null;

    try {
      CompletableFuture<List<Frequency>> futureList =
          queryGateway.query(
              new FindAllFrequencyQuery(), ResponseTypes.multipleInstancesOf(Frequency.class));
  ...
  /**
   * assign Value on Frequency
   *
   * @param command AssignValueToFrequencyCommand
   * @exception ProcessingException
   */
  public void assignValue(AssignValueToFrequencyCommand command) throws ProcessingException {

    // --------------------------------------------
    // load the parent
    // --------------------------------------------
    load(command.getFrequencyId());
    ...
    FrequencyBusinessDelegate parentDelegate = FrequencyBusinessDelegate.getFrequencyInstance();
   ...
   * @exception ProcessingException
   */
  public void unAssignValue(UnAssignValueFromFrequencyCommand command) throws ProcessingException {
    ...
    try {
      ...
      final String msg = "Failed to unassign Value on Frequency";
   ...
   * @return Frequency
   */
  private Frequency load(UUID id) throws ProcessingException {
    frequency =
        FrequencyBusinessDelegate.getFrequencyInstance()
            .getFrequency(new FrequencyFetchOneSummary(id));
    return frequency;
  }
