 ...
 *   <li>Improving the available of MonthDay related services in the case of a MonthDay business
 *       related service failing.
 *   <li>Exposes a simpler, uniform MonthDay interface to the business tier, making it easy for
 *       clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill MonthDay business related
 *       services.
 * </ol>
 *
 * <p>
 ...
 */
public class MonthDayBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createMonthDay(CreateMonthDayCommand command)
    ...
    try {
      ...
      // --------------------------------------
      if (command.getMonthDayId() == null) command.setMonthDayId(UUID.randomUUID());
      ...
      // ---------------------------------------
      // issue the CreateMonthDayCommand - by convention the future return value for a create
      // command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateMonthDayCommand of MonthDay is " + command);
  ...
  public MonthDay getMonthDay(MonthDayFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<MonthDay> futureEntity =
          queryGateway.query(
              new FindMonthDayQuery(new LoadMonthDayFilter(summary.getMonthDayId())),
   ...
   */
  public List<MonthDay> getAllMonthDay() throws ProcessingException {
    List<MonthDay> list = null;

    try {
      CompletableFuture<List<MonthDay>> futureList =
          queryGateway.query(
              new FindAllMonthDayQuery(), ResponseTypes.multipleInstancesOf(MonthDay.class));
   ...
   * @return MonthDay
   */
  private MonthDay load(UUID id) throws ProcessingException {
    monthDay =
        MonthDayBusinessDelegate.getMonthDayInstance().getMonthDay(new MonthDayFetchOneSummary(id));
    return monthDay;
  }
