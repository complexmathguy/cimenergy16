 ...
 *   <li>Improving the available of DateTime related services in the case of a DateTime business
 *       related service failing.
 *   <li>Exposes a simpler, uniform DateTime interface to the business tier, making it easy for
 *       clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill DateTime business related
 *       services.
 * </ol>
 *
 * <p>
 ...
 */
public class DateTimeBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createDateTime(CreateDateTimeCommand command)
    ...
    try {
      ...
      // --------------------------------------
      if (command.getDateTimeId() == null) command.setDateTimeId(UUID.randomUUID());
      ...
      // ---------------------------------------
      // issue the CreateDateTimeCommand - by convention the future return value for a create
      // command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateDateTimeCommand of DateTime is " + command);
  ...
  public DateTime getDateTime(DateTimeFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<DateTime> futureEntity =
          queryGateway.query(
              new FindDateTimeQuery(new LoadDateTimeFilter(summary.getDateTimeId())),
   ...
   */
  public List<DateTime> getAllDateTime() throws ProcessingException {
    List<DateTime> list = null;

    try {
      CompletableFuture<List<DateTime>> futureList =
          queryGateway.query(
              new FindAllDateTimeQuery(), ResponseTypes.multipleInstancesOf(DateTime.class));
   ...
   * @return DateTime
   */
  private DateTime load(UUID id) throws ProcessingException {
    dateTime =
        DateTimeBusinessDelegate.getDateTimeInstance().getDateTime(new DateTimeFetchOneSummary(id));
    return dateTime;
  }
