 ...
 *   <li>Improving the available of BooleanProxy related services in the case of a BooleanProxy
 *       business related service failing.
 *   <li>Exposes a simpler, uniform BooleanProxy interface to the business tier, making it easy for
 *       clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill BooleanProxy business
 *       related services.
 * </ol>
 *
 * <p>
 ...
 */
public class BooleanProxyBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createBooleanProxy(CreateBooleanProxyCommand command)
    ...
    try {
      ...
      // --------------------------------------
      if (command.getBooleanProxyId() == null) command.setBooleanProxyId(UUID.randomUUID());
      ...
      // ---------------------------------------
      // issue the CreateBooleanProxyCommand - by convention the future return value for a create
      // command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateBooleanProxyCommand of BooleanProxy is "
              + command);
  ...
  public CompletableFuture<Void> delete(DeleteBooleanProxyCommand command)
    ...
    try {
      ...
      final String errMsg =
          "Unable to delete BooleanProxy using Id = " + command.getBooleanProxyId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
  ...
  public BooleanProxy getBooleanProxy(BooleanProxyFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<BooleanProxy> futureEntity =
          queryGateway.query(
              new FindBooleanProxyQuery(new LoadBooleanProxyFilter(summary.getBooleanProxyId())),
   ...
   */
  public List<BooleanProxy> getAllBooleanProxy() throws ProcessingException {
    List<BooleanProxy> list = null;

    try {
      CompletableFuture<List<BooleanProxy>> futureList =
          queryGateway.query(
   ...
   * @return BooleanProxy
   */
  private BooleanProxy load(UUID id) throws ProcessingException {
    booleanProxy =
        BooleanProxyBusinessDelegate.getBooleanProxyInstance()
            .getBooleanProxy(new BooleanProxyFetchOneSummary(id));
    return booleanProxy;
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(BooleanProxyBusinessDelegate.class.getName());
}
