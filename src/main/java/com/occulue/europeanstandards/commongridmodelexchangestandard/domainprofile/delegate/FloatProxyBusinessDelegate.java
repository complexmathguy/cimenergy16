 ...
 *   <li>Improving the available of FloatProxy related services in the case of a FloatProxy business
 *       related service failing.
 *   <li>Exposes a simpler, uniform FloatProxy interface to the business tier, making it easy for
 *       clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill FloatProxy business
 *       related services.
 * </ol>
 *
 * <p>
 ...
 */
public class FloatProxyBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createFloatProxy(CreateFloatProxyCommand command)
    ...
    try {
      ...
      // --------------------------------------
      if (command.getFloatProxyId() == null) command.setFloatProxyId(UUID.randomUUID());
      ...
      // ---------------------------------------
      // issue the CreateFloatProxyCommand - by convention the future return value for a create
      // command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateFloatProxyCommand of FloatProxy is " + command);
  ...
  public FloatProxy getFloatProxy(FloatProxyFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<FloatProxy> futureEntity =
          queryGateway.query(
              new FindFloatProxyQuery(new LoadFloatProxyFilter(summary.getFloatProxyId())),
              ResponseTypes.instanceOf(FloatProxy.class));
   ...
   */
  public List<FloatProxy> getAllFloatProxy() throws ProcessingException {
    List<FloatProxy> list = null;

    try {
      CompletableFuture<List<FloatProxy>> futureList =
          queryGateway.query(
              new FindAllFloatProxyQuery(), ResponseTypes.multipleInstancesOf(FloatProxy.class));
   ...
   * @return FloatProxy
   */
  private FloatProxy load(UUID id) throws ProcessingException {
    floatProxy =
        FloatProxyBusinessDelegate.getFloatProxyInstance()
            .getFloatProxy(new FloatProxyFetchOneSummary(id));
    return floatProxy;
  }
