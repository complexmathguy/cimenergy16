 ...
 *   <li>Improving the available of PerCent related services in the case of a PerCent business
 *       related service failing.
 *   <li>Exposes a simpler, uniform PerCent interface to the business tier, making it easy for
 *       clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill PerCent business related
 *       services.
 * </ol>
 *
 * <p>
 ...
 */
public class PerCentBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createPerCent(CreatePerCentCommand command)
    ...
    try {
      ...
      // --------------------------------------
      if (command.getPerCentId() == null) command.setPerCentId(UUID.randomUUID());
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreatePerCentCommand of PerCent is " + command);
  ...
  public PerCent getPerCent(PerCentFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<PerCent> futureEntity =
          queryGateway.query(
              new FindPerCentQuery(new LoadPerCentFilter(summary.getPerCentId())),
   ...
   */
  public List<PerCent> getAllPerCent() throws ProcessingException {
    List<PerCent> list = null;

    try {
      CompletableFuture<List<PerCent>> futureList =
          queryGateway.query(
              new FindAllPerCentQuery(), ResponseTypes.multipleInstancesOf(PerCent.class));
  ...
  private PerCent load(UUID id) throws ProcessingException {
    perCent =
        PerCentBusinessDelegate.getPerCentInstance().getPerCent(new PerCentFetchOneSummary(id));
    return perCent;
  }
