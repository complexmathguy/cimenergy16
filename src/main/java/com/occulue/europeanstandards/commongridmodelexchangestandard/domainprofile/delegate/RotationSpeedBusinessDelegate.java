 ...
 *   <li>Improving the available of RotationSpeed related services in the case of a RotationSpeed
 *       business related service failing.
 *   <li>Exposes a simpler, uniform RotationSpeed interface to the business tier, making it easy for
 *       clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill RotationSpeed business
 *       related services.
 * </ol>
 *
 * <p>
 ...
 */
public class RotationSpeedBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createRotationSpeed(CreateRotationSpeedCommand command)
    ...
    try {
      ...
      // --------------------------------------
      if (command.getRotationSpeedId() == null) command.setRotationSpeedId(UUID.randomUUID());
      ...
      // ---------------------------------------
      // issue the CreateRotationSpeedCommand - by convention the future return value for a create
      // command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateRotationSpeedCommand of RotationSpeed is "
              + command);
  ...
  public CompletableFuture<Void> delete(DeleteRotationSpeedCommand command)
    ...
    try {
      ...
      final String errMsg =
          "Unable to delete RotationSpeed using Id = " + command.getRotationSpeedId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
  ...
  public RotationSpeed getRotationSpeed(RotationSpeedFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<RotationSpeed> futureEntity =
          queryGateway.query(
              new FindRotationSpeedQuery(new LoadRotationSpeedFilter(summary.getRotationSpeedId())),
              ResponseTypes.instanceOf(RotationSpeed.class));

      entity = futureEntity.get();
    } catch (Exception exc) {
      final String errMsg =
          "Unable to locate RotationSpeed with id " + summary.getRotationSpeedId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
   ...
   */
  public List<RotationSpeed> getAllRotationSpeed() throws ProcessingException {
    List<RotationSpeed> list = null;

    try {
      CompletableFuture<List<RotationSpeed>> futureList =
          queryGateway.query(
  ...
  /**
   * assign Value on RotationSpeed
   *
   * @param command AssignValueToRotationSpeedCommand
   * @exception ProcessingException
   */
  public void assignValue(AssignValueToRotationSpeedCommand command) throws ProcessingException {

    // --------------------------------------------
    // load the parent
    // --------------------------------------------
    load(command.getRotationSpeedId());
    ...
    RotationSpeedBusinessDelegate parentDelegate =
   ...
   * @exception ProcessingException
   */
  public void unAssignValue(UnAssignValueFromRotationSpeedCommand command)
    ...
    try {
      ...
      final String msg = "Failed to unassign Value on RotationSpeed";
   ...
   */
  private RotationSpeed load(UUID id) throws ProcessingException {
    rotationSpeed =
        RotationSpeedBusinessDelegate.getRotationSpeedInstance()
            .getRotationSpeed(new RotationSpeedFetchOneSummary(id));
    return rotationSpeed;
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(RotationSpeedBusinessDelegate.class.getName());
}
