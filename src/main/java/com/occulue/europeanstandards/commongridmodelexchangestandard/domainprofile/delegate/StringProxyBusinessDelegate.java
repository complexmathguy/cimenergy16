 ...
 *   <li>Improving the available of StringProxy related services in the case of a StringProxy
 *       business related service failing.
 *   <li>Exposes a simpler, uniform StringProxy interface to the business tier, making it easy for
 *       clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill StringProxy business
 *       related services.
 * </ol>
 *
 * <p>
 ...
 */
public class StringProxyBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createStringProxy(CreateStringProxyCommand command)
    ...
    try {
      ...
      // --------------------------------------
      if (command.getStringProxyId() == null) command.setStringProxyId(UUID.randomUUID());
      ...
      // ---------------------------------------
      // issue the CreateStringProxyCommand - by convention the future return value for a create
      // command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateStringProxyCommand of StringProxy is " + command);
  ...
  public StringProxy getStringProxy(StringProxyFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<StringProxy> futureEntity =
          queryGateway.query(
              new FindStringProxyQuery(new LoadStringProxyFilter(summary.getStringProxyId())),
   ...
   */
  public List<StringProxy> getAllStringProxy() throws ProcessingException {
    List<StringProxy> list = null;

    try {
      CompletableFuture<List<StringProxy>> futureList =
          queryGateway.query(
              new FindAllStringProxyQuery(), ResponseTypes.multipleInstancesOf(StringProxy.class));
   ...
   * @return StringProxy
   */
  private StringProxy load(UUID id) throws ProcessingException {
    stringProxy =
        StringProxyBusinessDelegate.getStringProxyInstance()
            .getStringProxy(new StringProxyFetchOneSummary(id));
    return stringProxy;
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(StringProxyBusinessDelegate.class.getName());
}
