 ...
 *   <li>Improving the available of PU related services in the case of a PU business related service
 *       failing.
 *   <li>Exposes a simpler, uniform PU interface to the business tier, making it easy for clients to
 *       consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill PU business related
 *       services.
 * </ol>
 *
 * <p>
 ...
 */
public class PUBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createPU(CreatePUCommand command)
    ...
    try {
      ...
      // --------------------------------------
      if (command.getPUId() == null) command.setPUId(UUID.randomUUID());
   ...
   * @exception IllegalArgumentException
   */
  public PU getPU(PUFetchOneSummary summary) throws ProcessingException, IllegalArgumentException {

    if (summary == null) throw new IllegalArgumentException("PUFetchOneSummary arg cannot be null");
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<PU> futureEntity =
          queryGateway.query(
              new FindPUQuery(new LoadPUFilter(summary.getPUId())),
   ...
   */
  public List<PU> getAllPU() throws ProcessingException {
    List<PU> list = null;

    try {
      CompletableFuture<List<PU>> futureList =
          queryGateway.query(new FindAllPUQuery(), ResponseTypes.multipleInstancesOf(PU.class));
   ...
   * @exception ProcessingException
   */
  public void assignValue(AssignValueToPUCommand command) throws ProcessingException {

    // --------------------------------------------
    // load the parent
    // --------------------------------------------
    load(command.getPUId());
    ...
    PUBusinessDelegate parentDelegate = PUBusinessDelegate.getPUInstance();
   ...
   * @exception ProcessingException
   */
  public void unAssignValue(UnAssignValueFromPUCommand command) throws ProcessingException {
    ...
    try {
      ...
      final String msg = "Failed to unassign Value on PU";
   ...
   * @return PU
   */
  private PU load(UUID id) throws ProcessingException {
