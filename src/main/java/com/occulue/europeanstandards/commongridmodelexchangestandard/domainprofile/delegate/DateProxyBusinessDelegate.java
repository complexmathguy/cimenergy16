 ...
 *   <li>Improving the available of DateProxy related services in the case of a DateProxy business
 *       related service failing.
 *   <li>Exposes a simpler, uniform DateProxy interface to the business tier, making it easy for
 *       clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill DateProxy business related
 *       services.
 * </ol>
 *
 * <p>
 ...
 */
public class DateProxyBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createDateProxy(CreateDateProxyCommand command)
    ...
    try {
      ...
      // --------------------------------------
      if (command.getDateProxyId() == null) command.setDateProxyId(UUID.randomUUID());
      ...
      // ---------------------------------------
      // issue the CreateDateProxyCommand - by convention the future return value for a create
      // command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateDateProxyCommand of DateProxy is " + command);
  ...
  public DateProxy getDateProxy(DateProxyFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<DateProxy> futureEntity =
          queryGateway.query(
              new FindDateProxyQuery(new LoadDateProxyFilter(summary.getDateProxyId())),
   ...
   */
  public List<DateProxy> getAllDateProxy() throws ProcessingException {
    List<DateProxy> list = null;

    try {
      CompletableFuture<List<DateProxy>> futureList =
          queryGateway.query(
              new FindAllDateProxyQuery(), ResponseTypes.multipleInstancesOf(DateProxy.class));
   ...
   * @return DateProxy
   */
  private DateProxy load(UUID id) throws ProcessingException {
    dateProxy =
        DateProxyBusinessDelegate.getDateProxyInstance()
            .getDateProxy(new DateProxyFetchOneSummary(id));
    return dateProxy;
  }
