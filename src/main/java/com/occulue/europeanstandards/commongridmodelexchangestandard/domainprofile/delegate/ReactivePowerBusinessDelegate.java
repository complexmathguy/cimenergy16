 ...
 *   <li>Improving the available of ReactivePower related services in the case of a ReactivePower
 *       business related service failing.
 *   <li>Exposes a simpler, uniform ReactivePower interface to the business tier, making it easy for
 *       clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill ReactivePower business
 *       related services.
 * </ol>
 *
 * <p>
 ...
 */
public class ReactivePowerBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createReactivePower(CreateReactivePowerCommand command)
    ...
    try {
      ...
      // --------------------------------------
      if (command.getReactivePowerId() == null) command.setReactivePowerId(UUID.randomUUID());
      ...
      // ---------------------------------------
      // issue the CreateReactivePowerCommand - by convention the future return value for a create
      // command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateReactivePowerCommand of ReactivePower is "
              + command);
  ...
  public CompletableFuture<Void> delete(DeleteReactivePowerCommand command)
    ...
    try {
      ...
      final String errMsg =
          "Unable to delete ReactivePower using Id = " + command.getReactivePowerId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
  ...
  public ReactivePower getReactivePower(ReactivePowerFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<ReactivePower> futureEntity =
          queryGateway.query(
              new FindReactivePowerQuery(new LoadReactivePowerFilter(summary.getReactivePowerId())),
              ResponseTypes.instanceOf(ReactivePower.class));

      entity = futureEntity.get();
    } catch (Exception exc) {
      final String errMsg =
          "Unable to locate ReactivePower with id " + summary.getReactivePowerId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
   ...
   */
  public List<ReactivePower> getAllReactivePower() throws ProcessingException {
    List<ReactivePower> list = null;

    try {
      CompletableFuture<List<ReactivePower>> futureList =
          queryGateway.query(
  ...
  /**
   * assign Value on ReactivePower
   *
   * @param command AssignValueToReactivePowerCommand
   * @exception ProcessingException
   */
  public void assignValue(AssignValueToReactivePowerCommand command) throws ProcessingException {

    // --------------------------------------------
    // load the parent
    // --------------------------------------------
    load(command.getReactivePowerId());
    ...
    ReactivePowerBusinessDelegate parentDelegate =
   ...
   * @exception ProcessingException
   */
  public void unAssignValue(UnAssignValueFromReactivePowerCommand command)
    ...
    try {
      ...
      final String msg = "Failed to unassign Value on ReactivePower";
   ...
   */
  private ReactivePower load(UUID id) throws ProcessingException {
    reactivePower =
        ReactivePowerBusinessDelegate.getReactivePowerInstance()
            .getReactivePower(new ReactivePowerFetchOneSummary(id));
    return reactivePower;
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(ReactivePowerBusinessDelegate.class.getName());
}
