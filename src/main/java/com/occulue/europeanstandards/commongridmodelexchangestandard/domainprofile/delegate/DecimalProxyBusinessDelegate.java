 ...
 *   <li>Improving the available of DecimalProxy related services in the case of a DecimalProxy
 *       business related service failing.
 *   <li>Exposes a simpler, uniform DecimalProxy interface to the business tier, making it easy for
 *       clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill DecimalProxy business
 *       related services.
 * </ol>
 *
 * <p>
 ...
 */
public class DecimalProxyBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createDecimalProxy(CreateDecimalProxyCommand command)
    ...
    try {
      ...
      // --------------------------------------
      if (command.getDecimalProxyId() == null) command.setDecimalProxyId(UUID.randomUUID());
      ...
      // ---------------------------------------
      // issue the CreateDecimalProxyCommand - by convention the future return value for a create
      // command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateDecimalProxyCommand of DecimalProxy is "
              + command);
  ...
  public CompletableFuture<Void> delete(DeleteDecimalProxyCommand command)
    ...
    try {
      ...
      final String errMsg =
          "Unable to delete DecimalProxy using Id = " + command.getDecimalProxyId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
  ...
  public DecimalProxy getDecimalProxy(DecimalProxyFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<DecimalProxy> futureEntity =
          queryGateway.query(
              new FindDecimalProxyQuery(new LoadDecimalProxyFilter(summary.getDecimalProxyId())),
   ...
   */
  public List<DecimalProxy> getAllDecimalProxy() throws ProcessingException {
    List<DecimalProxy> list = null;

    try {
      CompletableFuture<List<DecimalProxy>> futureList =
          queryGateway.query(
   ...
   * @return DecimalProxy
   */
  private DecimalProxy load(UUID id) throws ProcessingException {
    decimalProxy =
        DecimalProxyBusinessDelegate.getDecimalProxyInstance()
            .getDecimalProxy(new DecimalProxyFetchOneSummary(id));
    return decimalProxy;
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(DecimalProxyBusinessDelegate.class.getName());
}
