...
import com.occulue.europeanstandards.commongridmodelexchangestandard.topologyprofile.dc.validator.*;
 ...
 *   <li>Improving the available of DCTopologicalNode related services in the case of a
 *       DCTopologicalNode business related service failing.
 *   <li>Exposes a simpler, uniform DCTopologicalNode interface to the business tier, making it easy
 *       for clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill DCTopologicalNode business
 *       related services.
 * </ol>
 *
 * <p>
 ...
 */
public class DCTopologicalNodeBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createDCTopologicalNode(CreateDCTopologicalNodeCommand command)
    ...
    try {
      ...
      // ---------------------------------------
      // issue the CreateDCTopologicalNodeCommand - by convention the future return value for a
      // create command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateDCTopologicalNodeCommand of DCTopologicalNode is "
              + command);
  ...
  public CompletableFuture<Void> delete(DeleteDCTopologicalNodeCommand command)
    ...
    try {
      ...
      final String errMsg =
          "Unable to delete DCTopologicalNode using Id = " + command.getDCTopologicalNodeId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
  ...
  public DCTopologicalNode getDCTopologicalNode(DCTopologicalNodeFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<DCTopologicalNode> futureEntity =
          queryGateway.query(
              new FindDCTopologicalNodeQuery(
                  new LoadDCTopologicalNodeFilter(summary.getDCTopologicalNodeId())),
              ResponseTypes.instanceOf(DCTopologicalNode.class));

      entity = futureEntity.get();
    } catch (Exception exc) {
      final String errMsg =
          "Unable to locate DCTopologicalNode with id " + summary.getDCTopologicalNodeId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
   ...
   */
  public List<DCTopologicalNode> getAllDCTopologicalNode() throws ProcessingException {
    List<DCTopologicalNode> list = null;

    try {
      CompletableFuture<List<DCTopologicalNode>> futureList =
          queryGateway.query(
              new FindAllDCTopologicalNodeQuery(),
   ...
   * @exception ProcessingException
   */
  public void addToDCTopologicalNode(AssignDCTopologicalNodeToDCTopologicalNodeCommand command)
    ...
    com.occulue.europeanstandards.commongridmodelexchangestandard.topologyprofile.dc.delegate
            .DCTopologicalNodeBusinessDelegate
        childDelegate =
            com.occulue.europeanstandards.commongridmodelexchangestandard.topologyprofile.dc
                .delegate.DCTopologicalNodeBusinessDelegate.getDCTopologicalNodeInstance();
    DCTopologicalNodeBusinessDelegate parentDelegate =
    ...
    try {
      ...
      final String msg =
          "Failed to add a DCTopologicalNode as DCTopologicalNode to DCTopologicalNode";
  ...
  public void removeFromDCTopologicalNode(
      RemoveDCTopologicalNodeFromDCTopologicalNodeCommand command) throws ProcessingException {
  ...
  private DCTopologicalNode load(UUID id) throws ProcessingException {
    dCTopologicalNode =
        DCTopologicalNodeBusinessDelegate.getDCTopologicalNodeInstance()
            .getDCTopologicalNode(new DCTopologicalNodeFetchOneSummary(id));
    return dCTopologicalNode;
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(DCTopologicalNodeBusinessDelegate.class.getName());
}
