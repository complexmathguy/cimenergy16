 ...
 *   <li>Improving the available of DCDisconnector related services in the case of a DCDisconnector
 *       business related service failing.
 *   <li>Exposes a simpler, uniform DCDisconnector interface to the business tier, making it easy
 *       for clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill DCDisconnector business
 *       related services.
 * </ol>
 *
 * <p>
 ...
 */
public class DCDisconnectorBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createDCDisconnector(CreateDCDisconnectorCommand command)
    ...
    try {
      ...
      // --------------------------------------
      if (command.getDCDisconnectorId() == null) command.setDCDisconnectorId(UUID.randomUUID());
      ...
      // ---------------------------------------
      // issue the CreateDCDisconnectorCommand - by convention the future return value for a create
      // command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateDCDisconnectorCommand of DCDisconnector is "
              + command);
  ...
  public CompletableFuture<Void> delete(DeleteDCDisconnectorCommand command)
    ...
    try {
      ...
      final String errMsg =
          "Unable to delete DCDisconnector using Id = " + command.getDCDisconnectorId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
  ...
  public DCDisconnector getDCDisconnector(DCDisconnectorFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<DCDisconnector> futureEntity =
          queryGateway.query(
              new FindDCDisconnectorQuery(
                  new LoadDCDisconnectorFilter(summary.getDCDisconnectorId())),
              ResponseTypes.instanceOf(DCDisconnector.class));

      entity = futureEntity.get();
    } catch (Exception exc) {
      final String errMsg =
          "Unable to locate DCDisconnector with id " + summary.getDCDisconnectorId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
   ...
   */
  public List<DCDisconnector> getAllDCDisconnector() throws ProcessingException {
    List<DCDisconnector> list = null;

    try {
      CompletableFuture<List<DCDisconnector>> futureList =
          queryGateway.query(
   ...
   */
  private DCDisconnector load(UUID id) throws ProcessingException {
    dCDisconnector =
        DCDisconnectorBusinessDelegate.getDCDisconnectorInstance()
            .getDCDisconnector(new DCDisconnectorFetchOneSummary(id));
    return dCDisconnector;
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(DCDisconnectorBusinessDelegate.class.getName());
}
