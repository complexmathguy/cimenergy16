 ...
 *   <li>Improving the available of SolarGeneratingUnit related services in the case of a
 *       SolarGeneratingUnit business related service failing.
 *   <li>Exposes a simpler, uniform SolarGeneratingUnit interface to the business tier, making it
 *       easy for clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill SolarGeneratingUnit
 ...
 */
public class SolarGeneratingUnitBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createSolarGeneratingUnit(CreateSolarGeneratingUnitCommand command)
    ...
    try {
      ...
      // ---------------------------------------
      // issue the CreateSolarGeneratingUnitCommand - by convention the future return value for a
      // create command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateSolarGeneratingUnitCommand of SolarGeneratingUnit is "
              + command);
  ...
  public CompletableFuture<Void> delete(DeleteSolarGeneratingUnitCommand command)
    ...
    try {
      ...
      final String errMsg =
          "Unable to delete SolarGeneratingUnit using Id = " + command.getSolarGeneratingUnitId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
  ...
  public SolarGeneratingUnit getSolarGeneratingUnit(SolarGeneratingUnitFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<SolarGeneratingUnit> futureEntity =
          queryGateway.query(
              new FindSolarGeneratingUnitQuery(
                  new LoadSolarGeneratingUnitFilter(summary.getSolarGeneratingUnitId())),
              ResponseTypes.instanceOf(SolarGeneratingUnit.class));

      entity = futureEntity.get();
    } catch (Exception exc) {
      final String errMsg =
          "Unable to locate SolarGeneratingUnit with id " + summary.getSolarGeneratingUnitId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
   ...
   */
  public List<SolarGeneratingUnit> getAllSolarGeneratingUnit() throws ProcessingException {
    List<SolarGeneratingUnit> list = null;

    try {
      CompletableFuture<List<SolarGeneratingUnit>> futureList =
          queryGateway.query(
   ...
   */
  private SolarGeneratingUnit load(UUID id) throws ProcessingException {
    solarGeneratingUnit =
        SolarGeneratingUnitBusinessDelegate.getSolarGeneratingUnitInstance()
            .getSolarGeneratingUnit(new SolarGeneratingUnitFetchOneSummary(id));
    return solarGeneratingUnit;
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(SolarGeneratingUnitBusinessDelegate.class.getName());
}
