 ...
 *   <li>Improving the available of Accumulator related services in the case of a Accumulator
 *       business related service failing.
 *   <li>Exposes a simpler, uniform Accumulator interface to the business tier, making it easy for
 *       clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill Accumulator business
 *       related services.
 * </ol>
 *
 * <p>
 ...
 */
public class AccumulatorBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createAccumulator(CreateAccumulatorCommand command)
    ...
    try {
      ...
      // --------------------------------------
      if (command.getAccumulatorId() == null) command.setAccumulatorId(UUID.randomUUID());
      ...
      // ---------------------------------------
      // issue the CreateAccumulatorCommand - by convention the future return value for a create
      // command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateAccumulatorCommand of Accumulator is " + command);
  ...
  public Accumulator getAccumulator(AccumulatorFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<Accumulator> futureEntity =
          queryGateway.query(
              new FindAccumulatorQuery(new LoadAccumulatorFilter(summary.getAccumulatorId())),
   ...
   */
  public List<Accumulator> getAllAccumulator() throws ProcessingException {
    List<Accumulator> list = null;

    try {
      CompletableFuture<List<Accumulator>> futureList =
          queryGateway.query(
              new FindAllAccumulatorQuery(), ResponseTypes.multipleInstancesOf(Accumulator.class));
   ...
   * @return Accumulator
   */
  private Accumulator load(UUID id) throws ProcessingException {
    accumulator =
        AccumulatorBusinessDelegate.getAccumulatorInstance()
            .getAccumulator(new AccumulatorFetchOneSummary(id));
    return accumulator;
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(AccumulatorBusinessDelegate.class.getName());
}
