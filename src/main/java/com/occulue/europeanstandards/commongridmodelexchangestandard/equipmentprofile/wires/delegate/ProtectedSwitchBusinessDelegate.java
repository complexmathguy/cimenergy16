 ...
 *   <li>Improving the available of ProtectedSwitch related services in the case of a
 *       ProtectedSwitch business related service failing.
 *   <li>Exposes a simpler, uniform ProtectedSwitch interface to the business tier, making it easy
 *       for clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill ProtectedSwitch business
 *       related services.
 * </ol>
 *
 * <p>
 ...
 */
public class ProtectedSwitchBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createProtectedSwitch(CreateProtectedSwitchCommand command)
    ...
    try {
      ...
      // --------------------------------------
      if (command.getProtectedSwitchId() == null) command.setProtectedSwitchId(UUID.randomUUID());
      ...
      // ---------------------------------------
      // issue the CreateProtectedSwitchCommand - by convention the future return value for a create
      // command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateProtectedSwitchCommand of ProtectedSwitch is "
              + command);
  ...
  public CompletableFuture<Void> delete(DeleteProtectedSwitchCommand command)
    ...
    try {
      ...
      final String errMsg =
          "Unable to delete ProtectedSwitch using Id = " + command.getProtectedSwitchId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
  ...
  public ProtectedSwitch getProtectedSwitch(ProtectedSwitchFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<ProtectedSwitch> futureEntity =
          queryGateway.query(
              new FindProtectedSwitchQuery(
                  new LoadProtectedSwitchFilter(summary.getProtectedSwitchId())),
              ResponseTypes.instanceOf(ProtectedSwitch.class));

      entity = futureEntity.get();
    } catch (Exception exc) {
      final String errMsg =
          "Unable to locate ProtectedSwitch with id " + summary.getProtectedSwitchId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
   ...
   */
  public List<ProtectedSwitch> getAllProtectedSwitch() throws ProcessingException {
    List<ProtectedSwitch> list = null;

    try {
      CompletableFuture<List<ProtectedSwitch>> futureList =
          queryGateway.query(
   ...
   */
  private ProtectedSwitch load(UUID id) throws ProcessingException {
    protectedSwitch =
        ProtectedSwitchBusinessDelegate.getProtectedSwitchInstance()
            .getProtectedSwitch(new ProtectedSwitchFetchOneSummary(id));
    return protectedSwitch;
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(ProtectedSwitchBusinessDelegate.class.getName());
}
