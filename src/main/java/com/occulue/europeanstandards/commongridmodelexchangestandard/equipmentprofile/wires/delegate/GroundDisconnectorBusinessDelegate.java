 ...
 *   <li>Improving the available of GroundDisconnector related services in the case of a
 *       GroundDisconnector business related service failing.
 *   <li>Exposes a simpler, uniform GroundDisconnector interface to the business tier, making it
 *       easy for clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill GroundDisconnector
 ...
 */
public class GroundDisconnectorBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createGroundDisconnector(CreateGroundDisconnectorCommand command)
    ...
    try {
      ...
      // ---------------------------------------
      // issue the CreateGroundDisconnectorCommand - by convention the future return value for a
      // create command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateGroundDisconnectorCommand of GroundDisconnector is "
              + command);
  ...
  public CompletableFuture<Void> delete(DeleteGroundDisconnectorCommand command)
    ...
    try {
      ...
      final String errMsg =
          "Unable to delete GroundDisconnector using Id = " + command.getGroundDisconnectorId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
  ...
  public GroundDisconnector getGroundDisconnector(GroundDisconnectorFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<GroundDisconnector> futureEntity =
          queryGateway.query(
              new FindGroundDisconnectorQuery(
                  new LoadGroundDisconnectorFilter(summary.getGroundDisconnectorId())),
              ResponseTypes.instanceOf(GroundDisconnector.class));

      entity = futureEntity.get();
    } catch (Exception exc) {
      final String errMsg =
          "Unable to locate GroundDisconnector with id " + summary.getGroundDisconnectorId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
   ...
   */
  public List<GroundDisconnector> getAllGroundDisconnector() throws ProcessingException {
    List<GroundDisconnector> list = null;

    try {
      CompletableFuture<List<GroundDisconnector>> futureList =
          queryGateway.query(
   ...
   */
  private GroundDisconnector load(UUID id) throws ProcessingException {
    groundDisconnector =
        GroundDisconnectorBusinessDelegate.getGroundDisconnectorInstance()
            .getGroundDisconnector(new GroundDisconnectorFetchOneSummary(id));
    return groundDisconnector;
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(GroundDisconnectorBusinessDelegate.class.getName());
}
