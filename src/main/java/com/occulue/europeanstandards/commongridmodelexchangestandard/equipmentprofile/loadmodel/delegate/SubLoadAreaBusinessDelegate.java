 ...
 *   <li>Improving the available of SubLoadArea related services in the case of a SubLoadArea
 *       business related service failing.
 *   <li>Exposes a simpler, uniform SubLoadArea interface to the business tier, making it easy for
 *       clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill SubLoadArea business
 *       related services.
 * </ol>
 *
 * <p>
 ...
 */
public class SubLoadAreaBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createSubLoadArea(CreateSubLoadAreaCommand command)
    ...
    try {
      ...
      // --------------------------------------
      if (command.getSubLoadAreaId() == null) command.setSubLoadAreaId(UUID.randomUUID());
      ...
      // ---------------------------------------
      // issue the CreateSubLoadAreaCommand - by convention the future return value for a create
      // command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateSubLoadAreaCommand of SubLoadArea is " + command);
  ...
  public SubLoadArea getSubLoadArea(SubLoadAreaFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<SubLoadArea> futureEntity =
          queryGateway.query(
              new FindSubLoadAreaQuery(new LoadSubLoadAreaFilter(summary.getSubLoadAreaId())),
   ...
   */
  public List<SubLoadArea> getAllSubLoadArea() throws ProcessingException {
    List<SubLoadArea> list = null;

    try {
      CompletableFuture<List<SubLoadArea>> futureList =
          queryGateway.query(
              new FindAllSubLoadAreaQuery(), ResponseTypes.multipleInstancesOf(SubLoadArea.class));
   ...
   * @exception ProcessingException
   */
  public void addToSubLoadAreas(AssignSubLoadAreasToSubLoadAreaCommand command)
    ...
    com.occulue.europeanstandards.commongridmodelexchangestandard.equipmentprofile.loadmodel
            .delegate.SubLoadAreaBusinessDelegate
        childDelegate =
            com.occulue.europeanstandards.commongridmodelexchangestandard.equipmentprofile.loadmodel
                .delegate.SubLoadAreaBusinessDelegate.getSubLoadAreaInstance();
    SubLoadAreaBusinessDelegate parentDelegate =
   ...
   * @exception ProcessingException
   */
  public void removeFromSubLoadAreas(RemoveSubLoadAreasFromSubLoadAreaCommand command)
      throws ProcessingException {
   ...
   * @return SubLoadArea
   */
  private SubLoadArea load(UUID id) throws ProcessingException {
    subLoadArea =
        SubLoadAreaBusinessDelegate.getSubLoadAreaInstance()
            .getSubLoadArea(new SubLoadAreaFetchOneSummary(id));
    return subLoadArea;
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(SubLoadAreaBusinessDelegate.class.getName());
}
