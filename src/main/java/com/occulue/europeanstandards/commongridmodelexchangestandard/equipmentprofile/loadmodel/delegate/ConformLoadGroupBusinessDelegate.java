 ...
 *   <li>Improving the available of ConformLoadGroup related services in the case of a
 *       ConformLoadGroup business related service failing.
 *   <li>Exposes a simpler, uniform ConformLoadGroup interface to the business tier, making it easy
 *       for clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill ConformLoadGroup business
 *       related services.
 * </ol>
 *
 * <p>
 ...
 */
public class ConformLoadGroupBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createConformLoadGroup(CreateConformLoadGroupCommand command)
    ...
    try {
      ...
      // --------------------------------------
      if (command.getConformLoadGroupId() == null) command.setConformLoadGroupId(UUID.randomUUID());
      ...
      // ---------------------------------------
      // issue the CreateConformLoadGroupCommand - by convention the future return value for a
      // create command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateConformLoadGroupCommand of ConformLoadGroup is "
              + command);
  ...
  public CompletableFuture<Void> delete(DeleteConformLoadGroupCommand command)
    ...
    try {
      ...
      final String errMsg =
          "Unable to delete ConformLoadGroup using Id = " + command.getConformLoadGroupId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
  ...
  public ConformLoadGroup getConformLoadGroup(ConformLoadGroupFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<ConformLoadGroup> futureEntity =
          queryGateway.query(
              new FindConformLoadGroupQuery(
                  new LoadConformLoadGroupFilter(summary.getConformLoadGroupId())),
              ResponseTypes.instanceOf(ConformLoadGroup.class));

      entity = futureEntity.get();
    } catch (Exception exc) {
      final String errMsg =
          "Unable to locate ConformLoadGroup with id " + summary.getConformLoadGroupId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
   ...
   */
  public List<ConformLoadGroup> getAllConformLoadGroup() throws ProcessingException {
    List<ConformLoadGroup> list = null;

    try {
      CompletableFuture<List<ConformLoadGroup>> futureList =
          queryGateway.query(
   ...
   */
  private ConformLoadGroup load(UUID id) throws ProcessingException {
    conformLoadGroup =
        ConformLoadGroupBusinessDelegate.getConformLoadGroupInstance()
            .getConformLoadGroup(new ConformLoadGroupFetchOneSummary(id));
    return conformLoadGroup;
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(ConformLoadGroupBusinessDelegate.class.getName());
}
