 ...
 *   <li>Improving the available of DCSwitch related services in the case of a DCSwitch business
 *       related service failing.
 *   <li>Exposes a simpler, uniform DCSwitch interface to the business tier, making it easy for
 *       clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill DCSwitch business related
 *       services.
 * </ol>
 *
 * <p>
 ...
 */
public class DCSwitchBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createDCSwitch(CreateDCSwitchCommand command)
    ...
    try {
      ...
      // --------------------------------------
      if (command.getDCSwitchId() == null) command.setDCSwitchId(UUID.randomUUID());
      ...
      // ---------------------------------------
      // issue the CreateDCSwitchCommand - by convention the future return value for a create
      // command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateDCSwitchCommand of DCSwitch is " + command);
  ...
  public DCSwitch getDCSwitch(DCSwitchFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<DCSwitch> futureEntity =
          queryGateway.query(
              new FindDCSwitchQuery(new LoadDCSwitchFilter(summary.getDCSwitchId())),
   ...
   */
  public List<DCSwitch> getAllDCSwitch() throws ProcessingException {
    List<DCSwitch> list = null;

    try {
      CompletableFuture<List<DCSwitch>> futureList =
          queryGateway.query(
              new FindAllDCSwitchQuery(), ResponseTypes.multipleInstancesOf(DCSwitch.class));
   ...
   * @return DCSwitch
   */
  private DCSwitch load(UUID id) throws ProcessingException {
    dCSwitch =
        DCSwitchBusinessDelegate.getDCSwitchInstance().getDCSwitch(new DCSwitchFetchOneSummary(id));
    return dCSwitch;
  }
