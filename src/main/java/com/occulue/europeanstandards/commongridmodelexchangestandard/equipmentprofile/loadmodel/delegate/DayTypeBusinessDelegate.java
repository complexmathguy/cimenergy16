 ...
 *   <li>Improving the available of DayType related services in the case of a DayType business
 *       related service failing.
 *   <li>Exposes a simpler, uniform DayType interface to the business tier, making it easy for
 *       clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill DayType business related
 *       services.
 * </ol>
 *
 * <p>
 ...
 */
public class DayTypeBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createDayType(CreateDayTypeCommand command)
    ...
    try {
      ...
      // --------------------------------------
      if (command.getDayTypeId() == null) command.setDayTypeId(UUID.randomUUID());
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateDayTypeCommand of DayType is " + command);
  ...
  public DayType getDayType(DayTypeFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<DayType> futureEntity =
          queryGateway.query(
              new FindDayTypeQuery(new LoadDayTypeFilter(summary.getDayTypeId())),
   ...
   */
  public List<DayType> getAllDayType() throws ProcessingException {
    List<DayType> list = null;

    try {
      CompletableFuture<List<DayType>> futureList =
          queryGateway.query(
              new FindAllDayTypeQuery(), ResponseTypes.multipleInstancesOf(DayType.class));
   ...
   * @return DayType
   */
  private DayType load(UUID id) throws ProcessingException {
    dayType =
        DayTypeBusinessDelegate.getDayTypeInstance().getDayType(new DayTypeFetchOneSummary(id));
    return dayType;
  }
