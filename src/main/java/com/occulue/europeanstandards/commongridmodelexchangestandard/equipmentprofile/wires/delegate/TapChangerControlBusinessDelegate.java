 ...
 *   <li>Improving the available of TapChangerControl related services in the case of a
 *       TapChangerControl business related service failing.
 *   <li>Exposes a simpler, uniform TapChangerControl interface to the business tier, making it easy
 *       for clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill TapChangerControl business
 *       related services.
 * </ol>
 *
 * <p>
 ...
 */
public class TapChangerControlBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createTapChangerControl(CreateTapChangerControlCommand command)
    ...
    try {
      ...
      // ---------------------------------------
      // issue the CreateTapChangerControlCommand - by convention the future return value for a
      // create command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateTapChangerControlCommand of TapChangerControl is "
              + command);
  ...
  public CompletableFuture<Void> delete(DeleteTapChangerControlCommand command)
    ...
    try {
      ...
      final String errMsg =
          "Unable to delete TapChangerControl using Id = " + command.getTapChangerControlId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
  ...
  public TapChangerControl getTapChangerControl(TapChangerControlFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<TapChangerControl> futureEntity =
          queryGateway.query(
              new FindTapChangerControlQuery(
                  new LoadTapChangerControlFilter(summary.getTapChangerControlId())),
              ResponseTypes.instanceOf(TapChangerControl.class));

      entity = futureEntity.get();
    } catch (Exception exc) {
      final String errMsg =
          "Unable to locate TapChangerControl with id " + summary.getTapChangerControlId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
   ...
   */
  public List<TapChangerControl> getAllTapChangerControl() throws ProcessingException {
    List<TapChangerControl> list = null;

    try {
      CompletableFuture<List<TapChangerControl>> futureList =
          queryGateway.query(
   ...
   */
  private TapChangerControl load(UUID id) throws ProcessingException {
    tapChangerControl =
        TapChangerControlBusinessDelegate.getTapChangerControlInstance()
            .getTapChangerControl(new TapChangerControlFetchOneSummary(id));
    return tapChangerControl;
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(TapChangerControlBusinessDelegate.class.getName());
}
