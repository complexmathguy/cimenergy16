 ...
 *   <li>Improving the available of LoadBreakSwitch related services in the case of a
 *       LoadBreakSwitch business related service failing.
 *   <li>Exposes a simpler, uniform LoadBreakSwitch interface to the business tier, making it easy
 *       for clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill LoadBreakSwitch business
 *       related services.
 * </ol>
 *
 * <p>
 ...
 */
public class LoadBreakSwitchBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createLoadBreakSwitch(CreateLoadBreakSwitchCommand command)
    ...
    try {
      ...
      // --------------------------------------
      if (command.getLoadBreakSwitchId() == null) command.setLoadBreakSwitchId(UUID.randomUUID());
      ...
      // ---------------------------------------
      // issue the CreateLoadBreakSwitchCommand - by convention the future return value for a create
      // command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateLoadBreakSwitchCommand of LoadBreakSwitch is "
              + command);
  ...
  public CompletableFuture<Void> delete(DeleteLoadBreakSwitchCommand command)
    ...
    try {
      ...
      final String errMsg =
          "Unable to delete LoadBreakSwitch using Id = " + command.getLoadBreakSwitchId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
  ...
  public LoadBreakSwitch getLoadBreakSwitch(LoadBreakSwitchFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<LoadBreakSwitch> futureEntity =
          queryGateway.query(
              new FindLoadBreakSwitchQuery(
                  new LoadLoadBreakSwitchFilter(summary.getLoadBreakSwitchId())),
              ResponseTypes.instanceOf(LoadBreakSwitch.class));

      entity = futureEntity.get();
    } catch (Exception exc) {
      final String errMsg =
          "Unable to locate LoadBreakSwitch with id " + summary.getLoadBreakSwitchId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
   ...
   */
  public List<LoadBreakSwitch> getAllLoadBreakSwitch() throws ProcessingException {
    List<LoadBreakSwitch> list = null;

    try {
      CompletableFuture<List<LoadBreakSwitch>> futureList =
          queryGateway.query(
              new FindAllLoadBreakSwitchQuery(),
   ...
   */
  private LoadBreakSwitch load(UUID id) throws ProcessingException {
    loadBreakSwitch =
        LoadBreakSwitchBusinessDelegate.getLoadBreakSwitchInstance()
            .getLoadBreakSwitch(new LoadBreakSwitchFetchOneSummary(id));
    return loadBreakSwitch;
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(LoadBreakSwitchBusinessDelegate.class.getName());
}
