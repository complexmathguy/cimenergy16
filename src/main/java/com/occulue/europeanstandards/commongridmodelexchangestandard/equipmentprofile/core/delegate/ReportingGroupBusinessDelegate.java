 ...
 *   <li>Improving the available of ReportingGroup related services in the case of a ReportingGroup
 *       business related service failing.
 *   <li>Exposes a simpler, uniform ReportingGroup interface to the business tier, making it easy
 *       for clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill ReportingGroup business
 *       related services.
 * </ol>
 *
 * <p>
 ...
 */
public class ReportingGroupBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createReportingGroup(CreateReportingGroupCommand command)
    ...
    try {
      ...
      // --------------------------------------
      if (command.getReportingGroupId() == null) command.setReportingGroupId(UUID.randomUUID());
      ...
      // ---------------------------------------
      // issue the CreateReportingGroupCommand - by convention the future return value for a create
      // command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateReportingGroupCommand of ReportingGroup is "
              + command);
  ...
  public CompletableFuture<Void> delete(DeleteReportingGroupCommand command)
    ...
    try {
      ...
      final String errMsg =
          "Unable to delete ReportingGroup using Id = " + command.getReportingGroupId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
  ...
  public ReportingGroup getReportingGroup(ReportingGroupFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<ReportingGroup> futureEntity =
          queryGateway.query(
              new FindReportingGroupQuery(
                  new LoadReportingGroupFilter(summary.getReportingGroupId())),
              ResponseTypes.instanceOf(ReportingGroup.class));

      entity = futureEntity.get();
    } catch (Exception exc) {
      final String errMsg =
          "Unable to locate ReportingGroup with id " + summary.getReportingGroupId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
   ...
   */
  public List<ReportingGroup> getAllReportingGroup() throws ProcessingException {
    List<ReportingGroup> list = null;

    try {
      CompletableFuture<List<ReportingGroup>> futureList =
          queryGateway.query(
   ...
   */
  private ReportingGroup load(UUID id) throws ProcessingException {
    reportingGroup =
        ReportingGroupBusinessDelegate.getReportingGroupInstance()
            .getReportingGroup(new ReportingGroupFetchOneSummary(id));
    return reportingGroup;
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(ReportingGroupBusinessDelegate.class.getName());
}
