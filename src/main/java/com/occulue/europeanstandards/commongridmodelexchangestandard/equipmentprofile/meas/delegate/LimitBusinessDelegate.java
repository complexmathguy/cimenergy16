 ...
 *   <li>Improving the available of Limit related services in the case of a Limit business related
 *       service failing.
 *   <li>Exposes a simpler, uniform Limit interface to the business tier, making it easy for clients
 *       to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill Limit business related
 *       services.
 * </ol>
 *
 * <p>
 ...
 */
public class LimitBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createLimit(CreateLimitCommand command)
    ...
    try {
      ...
      // --------------------------------------
      if (command.getLimitId() == null) command.setLimitId(UUID.randomUUID());
      ...
      LOGGER.log(
          Level.INFO, "return from Command Gateway for CreateLimitCommand of Limit is " + command);
  ...
  public Limit getLimit(LimitFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<Limit> futureEntity =
          queryGateway.query(
              new FindLimitQuery(new LoadLimitFilter(summary.getLimitId())),
   ...
   */
  public List<Limit> getAllLimit() throws ProcessingException {
    List<Limit> list = null;

    try {
      CompletableFuture<List<Limit>> futureList =
          queryGateway.query(
              new FindAllLimitQuery(), ResponseTypes.multipleInstancesOf(Limit.class));
   ...
   * @return Limit
   */
  private Limit load(UUID id) throws ProcessingException {
