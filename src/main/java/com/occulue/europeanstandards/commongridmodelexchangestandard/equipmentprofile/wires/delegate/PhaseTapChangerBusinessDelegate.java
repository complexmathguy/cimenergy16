 ...
 *   <li>Improving the available of PhaseTapChanger related services in the case of a
 *       PhaseTapChanger business related service failing.
 *   <li>Exposes a simpler, uniform PhaseTapChanger interface to the business tier, making it easy
 *       for clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill PhaseTapChanger business
 *       related services.
 * </ol>
 *
 * <p>
 ...
 */
public class PhaseTapChangerBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createPhaseTapChanger(CreatePhaseTapChangerCommand command)
    ...
    try {
      ...
      // --------------------------------------
      if (command.getPhaseTapChangerId() == null) command.setPhaseTapChangerId(UUID.randomUUID());
      ...
      // ---------------------------------------
      // issue the CreatePhaseTapChangerCommand - by convention the future return value for a create
      // command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreatePhaseTapChangerCommand of PhaseTapChanger is "
              + command);
  ...
  public CompletableFuture<Void> delete(DeletePhaseTapChangerCommand command)
    ...
    try {
      ...
      final String errMsg =
          "Unable to delete PhaseTapChanger using Id = " + command.getPhaseTapChangerId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
  ...
  public PhaseTapChanger getPhaseTapChanger(PhaseTapChangerFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<PhaseTapChanger> futureEntity =
          queryGateway.query(
              new FindPhaseTapChangerQuery(
                  new LoadPhaseTapChangerFilter(summary.getPhaseTapChangerId())),
              ResponseTypes.instanceOf(PhaseTapChanger.class));

      entity = futureEntity.get();
    } catch (Exception exc) {
      final String errMsg =
          "Unable to locate PhaseTapChanger with id " + summary.getPhaseTapChangerId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
   ...
   */
  public List<PhaseTapChanger> getAllPhaseTapChanger() throws ProcessingException {
    List<PhaseTapChanger> list = null;

    try {
      CompletableFuture<List<PhaseTapChanger>> futureList =
          queryGateway.query(
  ...
  /**
   * assign PhaseTapChanger on PhaseTapChanger
   *
   * @param command AssignPhaseTapChangerToPhaseTapChangerCommand
   * @exception ProcessingException
   */
  public void assignPhaseTapChanger(AssignPhaseTapChangerToPhaseTapChangerCommand command)
      throws ProcessingException {

    // --------------------------------------------
    // load the parent
    // --------------------------------------------
    load(command.getPhaseTapChangerId());

    com.occulue.europeanstandards.commongridmodelexchangestandard.equipmentprofile.wires.delegate
            .PhaseTapChangerBusinessDelegate
        childDelegate =
            com.occulue.europeanstandards.commongridmodelexchangestandard.equipmentprofile.wires
                .delegate.PhaseTapChangerBusinessDelegate.getPhaseTapChangerInstance();
    PhaseTapChangerBusinessDelegate parentDelegate =
        PhaseTapChangerBusinessDelegate.getPhaseTapChangerInstance();
    UUID childId = command.getAssignment().getPhaseTapChangerId();
    PhaseTapChanger child = null;
    ...
    try {
      ...
      final String msg = "Failed to get PhaseTapChanger using id " + childId;
   ...
   * @exception ProcessingException
   */
  public void unAssignPhaseTapChanger(UnAssignPhaseTapChangerFromPhaseTapChangerCommand command)
      throws ProcessingException {
    ...
    try {
      ...
      final String msg = "Failed to unassign PhaseTapChanger on PhaseTapChanger";
   ...
   */
  private PhaseTapChanger load(UUID id) throws ProcessingException {
    phaseTapChanger =
        PhaseTapChangerBusinessDelegate.getPhaseTapChangerInstance()
            .getPhaseTapChanger(new PhaseTapChangerFetchOneSummary(id));
    return phaseTapChanger;
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(PhaseTapChangerBusinessDelegate.class.getName());
}
