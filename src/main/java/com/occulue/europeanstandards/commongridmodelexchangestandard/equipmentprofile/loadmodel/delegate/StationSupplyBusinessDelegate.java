 ...
 *   <li>Improving the available of StationSupply related services in the case of a StationSupply
 *       business related service failing.
 *   <li>Exposes a simpler, uniform StationSupply interface to the business tier, making it easy for
 *       clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill StationSupply business
 *       related services.
 * </ol>
 *
 * <p>
 ...
 */
public class StationSupplyBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createStationSupply(CreateStationSupplyCommand command)
    ...
    try {
      ...
      // --------------------------------------
      if (command.getStationSupplyId() == null) command.setStationSupplyId(UUID.randomUUID());
      ...
      // ---------------------------------------
      // issue the CreateStationSupplyCommand - by convention the future return value for a create
      // command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateStationSupplyCommand of StationSupply is "
              + command);
  ...
  public CompletableFuture<Void> delete(DeleteStationSupplyCommand command)
    ...
    try {
      ...
      final String errMsg =
          "Unable to delete StationSupply using Id = " + command.getStationSupplyId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
  ...
  public StationSupply getStationSupply(StationSupplyFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<StationSupply> futureEntity =
          queryGateway.query(
              new FindStationSupplyQuery(new LoadStationSupplyFilter(summary.getStationSupplyId())),
              ResponseTypes.instanceOf(StationSupply.class));

      entity = futureEntity.get();
    } catch (Exception exc) {
      final String errMsg =
          "Unable to locate StationSupply with id " + summary.getStationSupplyId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
   ...
   */
  public List<StationSupply> getAllStationSupply() throws ProcessingException {
    List<StationSupply> list = null;

    try {
      CompletableFuture<List<StationSupply>> futureList =
          queryGateway.query(
   ...
   */
  private StationSupply load(UUID id) throws ProcessingException {
    stationSupply =
        StationSupplyBusinessDelegate.getStationSupplyInstance()
            .getStationSupply(new StationSupplyFetchOneSummary(id));
    return stationSupply;
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(StationSupplyBusinessDelegate.class.getName());
}
