 ...
 *   <li>Improving the available of DCBreaker related services in the case of a DCBreaker business
 *       related service failing.
 *   <li>Exposes a simpler, uniform DCBreaker interface to the business tier, making it easy for
 *       clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill DCBreaker business related
 *       services.
 * </ol>
 *
 * <p>
 ...
 */
public class DCBreakerBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createDCBreaker(CreateDCBreakerCommand command)
    ...
    try {
      ...
      // --------------------------------------
      if (command.getDCBreakerId() == null) command.setDCBreakerId(UUID.randomUUID());
      ...
      // ---------------------------------------
      // issue the CreateDCBreakerCommand - by convention the future return value for a create
      // command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateDCBreakerCommand of DCBreaker is " + command);
  ...
  public DCBreaker getDCBreaker(DCBreakerFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<DCBreaker> futureEntity =
          queryGateway.query(
              new FindDCBreakerQuery(new LoadDCBreakerFilter(summary.getDCBreakerId())),
   ...
   */
  public List<DCBreaker> getAllDCBreaker() throws ProcessingException {
    List<DCBreaker> list = null;

    try {
      CompletableFuture<List<DCBreaker>> futureList =
          queryGateway.query(
              new FindAllDCBreakerQuery(), ResponseTypes.multipleInstancesOf(DCBreaker.class));
   ...
   * @return DCBreaker
   */
  private DCBreaker load(UUID id) throws ProcessingException {
    dCBreaker =
        DCBreakerBusinessDelegate.getDCBreakerInstance()
            .getDCBreaker(new DCBreakerFetchOneSummary(id));
    return dCBreaker;
  }
