 ...
 *   <li>Improving the available of NonConformLoadGroup related services in the case of a
 *       NonConformLoadGroup business related service failing.
 *   <li>Exposes a simpler, uniform NonConformLoadGroup interface to the business tier, making it
 *       easy for clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill NonConformLoadGroup
 ...
 */
public class NonConformLoadGroupBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createNonConformLoadGroup(CreateNonConformLoadGroupCommand command)
    ...
    try {
      ...
      // ---------------------------------------
      // issue the CreateNonConformLoadGroupCommand - by convention the future return value for a
      // create command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateNonConformLoadGroupCommand of NonConformLoadGroup is "
              + command);
  ...
  public CompletableFuture<Void> delete(DeleteNonConformLoadGroupCommand command)
    ...
    try {
      ...
      final String errMsg =
          "Unable to delete NonConformLoadGroup using Id = " + command.getNonConformLoadGroupId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
  ...
  public NonConformLoadGroup getNonConformLoadGroup(NonConformLoadGroupFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<NonConformLoadGroup> futureEntity =
          queryGateway.query(
              new FindNonConformLoadGroupQuery(
                  new LoadNonConformLoadGroupFilter(summary.getNonConformLoadGroupId())),
              ResponseTypes.instanceOf(NonConformLoadGroup.class));

      entity = futureEntity.get();
    } catch (Exception exc) {
      final String errMsg =
          "Unable to locate NonConformLoadGroup with id " + summary.getNonConformLoadGroupId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
   ...
   */
  public List<NonConformLoadGroup> getAllNonConformLoadGroup() throws ProcessingException {
    List<NonConformLoadGroup> list = null;

    try {
      CompletableFuture<List<NonConformLoadGroup>> futureList =
          queryGateway.query(
   ...
   */
  private NonConformLoadGroup load(UUID id) throws ProcessingException {
    nonConformLoadGroup =
        NonConformLoadGroupBusinessDelegate.getNonConformLoadGroupInstance()
            .getNonConformLoadGroup(new NonConformLoadGroupFetchOneSummary(id));
    return nonConformLoadGroup;
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(NonConformLoadGroupBusinessDelegate.class.getName());
}
