 ...
 *   <li>Improving the available of ThermalGeneratingUnit related services in the case of a
 *       ThermalGeneratingUnit business related service failing.
 *   <li>Exposes a simpler, uniform ThermalGeneratingUnit interface to the business tier, making it
 *       easy for clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill ThermalGeneratingUnit
 ...
 */
public class ThermalGeneratingUnitBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
   ...
   */
  public CompletableFuture<UUID> createThermalGeneratingUnit(
      CreateThermalGeneratingUnitCommand command)
    ...
    try {
      ...
      // ---------------------------------------
      // issue the CreateThermalGeneratingUnitCommand - by convention the future return value for a
      // create command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateThermalGeneratingUnitCommand of ThermalGeneratingUnit is "
              + command);
   ...
   * @exception IllegalArgumentException
   */
  public CompletableFuture<Void> updateThermalGeneratingUnit(
      UpdateThermalGeneratingUnitCommand command)
      throws ProcessingException, IllegalArgumentException {
  ...
  public CompletableFuture<Void> delete(DeleteThermalGeneratingUnitCommand command)
    ...
    try {
      ...
      final String errMsg =
          "Unable to delete ThermalGeneratingUnit using Id = "
              + command.getThermalGeneratingUnitId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
  ...
  public ThermalGeneratingUnit getThermalGeneratingUnit(
      ThermalGeneratingUnitFetchOneSummary summary)
      throws ProcessingException, IllegalArgumentException {
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<ThermalGeneratingUnit> futureEntity =
          queryGateway.query(
              new FindThermalGeneratingUnitQuery(
                  new LoadThermalGeneratingUnitFilter(summary.getThermalGeneratingUnitId())),
              ResponseTypes.instanceOf(ThermalGeneratingUnit.class));

      entity = futureEntity.get();
    } catch (Exception exc) {
      final String errMsg =
          "Unable to locate ThermalGeneratingUnit with id " + summary.getThermalGeneratingUnitId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
   ...
   */
  public List<ThermalGeneratingUnit> getAllThermalGeneratingUnit() throws ProcessingException {
    List<ThermalGeneratingUnit> list = null;

    try {
      CompletableFuture<List<ThermalGeneratingUnit>> futureList =
          queryGateway.query(
   ...
   */
  private ThermalGeneratingUnit load(UUID id) throws ProcessingException {
    thermalGeneratingUnit =
        ThermalGeneratingUnitBusinessDelegate.getThermalGeneratingUnitInstance()
            .getThermalGeneratingUnit(new ThermalGeneratingUnitFetchOneSummary(id));
    return thermalGeneratingUnit;
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(ThermalGeneratingUnitBusinessDelegate.class.getName());
}
