 ...
 *   <li>Improving the available of AnalogLimitSet related services in the case of a AnalogLimitSet
 *       business related service failing.
 *   <li>Exposes a simpler, uniform AnalogLimitSet interface to the business tier, making it easy
 *       for clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill AnalogLimitSet business
 *       related services.
 * </ol>
 *
 * <p>
 ...
 */
public class AnalogLimitSetBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createAnalogLimitSet(CreateAnalogLimitSetCommand command)
    ...
    try {
      ...
      // --------------------------------------
      if (command.getAnalogLimitSetId() == null) command.setAnalogLimitSetId(UUID.randomUUID());
      ...
      // ---------------------------------------
      // issue the CreateAnalogLimitSetCommand - by convention the future return value for a create
      // command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateAnalogLimitSetCommand of AnalogLimitSet is "
              + command);
  ...
  public CompletableFuture<Void> delete(DeleteAnalogLimitSetCommand command)
    ...
    try {
      ...
      final String errMsg =
          "Unable to delete AnalogLimitSet using Id = " + command.getAnalogLimitSetId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
  ...
  public AnalogLimitSet getAnalogLimitSet(AnalogLimitSetFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<AnalogLimitSet> futureEntity =
          queryGateway.query(
              new FindAnalogLimitSetQuery(
                  new LoadAnalogLimitSetFilter(summary.getAnalogLimitSetId())),
              ResponseTypes.instanceOf(AnalogLimitSet.class));

      entity = futureEntity.get();
    } catch (Exception exc) {
      final String errMsg =
          "Unable to locate AnalogLimitSet with id " + summary.getAnalogLimitSetId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
   ...
   */
  public List<AnalogLimitSet> getAllAnalogLimitSet() throws ProcessingException {
    List<AnalogLimitSet> list = null;

    try {
      CompletableFuture<List<AnalogLimitSet>> futureList =
          queryGateway.query(
  ...
  /**
   * add AnalogLimitSet to LimitSets
   *
   * @param command AssignLimitSetsToAnalogLimitSetCommand
   * @exception ProcessingException
   */
  public void addToLimitSets(AssignLimitSetsToAnalogLimitSetCommand command)
      throws ProcessingException {

    // -------------------------------------------
    // load the parent
    // -------------------------------------------
    load(command.getAnalogLimitSetId());

    com.occulue.europeanstandards.commongridmodelexchangestandard.equipmentprofile.meas.delegate
            .AnalogLimitSetBusinessDelegate
        childDelegate =
            com.occulue.europeanstandards.commongridmodelexchangestandard.equipmentprofile.meas
                .delegate.AnalogLimitSetBusinessDelegate.getAnalogLimitSetInstance();
    AnalogLimitSetBusinessDelegate parentDelegate =
        AnalogLimitSetBusinessDelegate.getAnalogLimitSetInstance();
    UUID childId = command.getAddTo().getAnalogLimitSetId();
    ...
    try {
      ...
      final String msg = "Failed to add a AnalogLimitSet as LimitSets to AnalogLimitSet";
  ...
  /**
   * remove AnalogLimitSet from LimitSets
   *
   * @param command RemoveLimitSetsFromAnalogLimitSetCommand
   * @exception ProcessingException
   */
  public void removeFromLimitSets(RemoveLimitSetsFromAnalogLimitSetCommand command)
      throws ProcessingException {
    ...
    com.occulue.europeanstandards.commongridmodelexchangestandard.equipmentprofile.meas.delegate
        ...
        childDelegate =
            com.occulue.europeanstandards.commongridmodelexchangestandard.equipmentprofile.meas
                .delegate.AnalogLimitSetBusinessDelegate.getAnalogLimitSetInstance();
    UUID childId = command.getRemoveFrom().getAnalogLimitSetId();
   ...
   */
  private AnalogLimitSet load(UUID id) throws ProcessingException {
    analogLimitSet =
        AnalogLimitSetBusinessDelegate.getAnalogLimitSetInstance()
            .getAnalogLimitSet(new AnalogLimitSetFetchOneSummary(id));
    return analogLimitSet;
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(AnalogLimitSetBusinessDelegate.class.getName());
}
