 ...
 *   <li>Improving the available of DCChopper related services in the case of a DCChopper business
 *       related service failing.
 *   <li>Exposes a simpler, uniform DCChopper interface to the business tier, making it easy for
 *       clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill DCChopper business related
 *       services.
 * </ol>
 *
 * <p>
 ...
 */
public class DCChopperBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createDCChopper(CreateDCChopperCommand command)
    ...
    try {
      ...
      // --------------------------------------
      if (command.getDCChopperId() == null) command.setDCChopperId(UUID.randomUUID());
      ...
      // ---------------------------------------
      // issue the CreateDCChopperCommand - by convention the future return value for a create
      // command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateDCChopperCommand of DCChopper is " + command);
  ...
  public DCChopper getDCChopper(DCChopperFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<DCChopper> futureEntity =
          queryGateway.query(
              new FindDCChopperQuery(new LoadDCChopperFilter(summary.getDCChopperId())),
   ...
   */
  public List<DCChopper> getAllDCChopper() throws ProcessingException {
    List<DCChopper> list = null;

    try {
      CompletableFuture<List<DCChopper>> futureList =
          queryGateway.query(
              new FindAllDCChopperQuery(), ResponseTypes.multipleInstancesOf(DCChopper.class));
   ...
   * @return DCChopper
   */
  private DCChopper load(UUID id) throws ProcessingException {
    dCChopper =
        DCChopperBusinessDelegate.getDCChopperInstance()
            .getDCChopper(new DCChopperFetchOneSummary(id));
    return dCChopper;
  }
