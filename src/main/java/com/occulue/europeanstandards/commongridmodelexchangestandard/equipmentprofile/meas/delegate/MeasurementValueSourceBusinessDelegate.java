 ...
 *   <li>Improving the available of MeasurementValueSource related services in the case of a
 *       MeasurementValueSource business related service failing.
 *   <li>Exposes a simpler, uniform MeasurementValueSource interface to the business tier, making it
 *       easy for clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill MeasurementValueSource
 ...
 */
public class MeasurementValueSourceBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
   ...
   */
  public CompletableFuture<UUID> createMeasurementValueSource(
      CreateMeasurementValueSourceCommand command)
    ...
    try {
      ...
      // ---------------------------------------
      // issue the CreateMeasurementValueSourceCommand - by convention the future return value for a
      // create command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateMeasurementValueSourceCommand of MeasurementValueSource is "
              + command);
   ...
   * @exception IllegalArgumentException
   */
  public CompletableFuture<Void> updateMeasurementValueSource(
      UpdateMeasurementValueSourceCommand command)
      throws ProcessingException, IllegalArgumentException {
  ...
  public CompletableFuture<Void> delete(DeleteMeasurementValueSourceCommand command)
    ...
    try {
      ...
      final String errMsg =
          "Unable to delete MeasurementValueSource using Id = "
              + command.getMeasurementValueSourceId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
  ...
  public MeasurementValueSource getMeasurementValueSource(
      MeasurementValueSourceFetchOneSummary summary)
      throws ProcessingException, IllegalArgumentException {

    if (summary == null)
      throw new IllegalArgumentException(
          "MeasurementValueSourceFetchOneSummary arg cannot be null");
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<MeasurementValueSource> futureEntity =
          queryGateway.query(
              new FindMeasurementValueSourceQuery(
                  new LoadMeasurementValueSourceFilter(summary.getMeasurementValueSourceId())),
              ResponseTypes.instanceOf(MeasurementValueSource.class));

      entity = futureEntity.get();
    } catch (Exception exc) {
      final String errMsg =
          "Unable to locate MeasurementValueSource with id "
   ...
   */
  public List<MeasurementValueSource> getAllMeasurementValueSource() throws ProcessingException {
    List<MeasurementValueSource> list = null;

    try {
      CompletableFuture<List<MeasurementValueSource>> futureList =
          queryGateway.query(
   ...
   */
  private MeasurementValueSource load(UUID id) throws ProcessingException {
    measurementValueSource =
        MeasurementValueSourceBusinessDelegate.getMeasurementValueSourceInstance()
            .getMeasurementValueSource(new MeasurementValueSourceFetchOneSummary(id));
    return measurementValueSource;
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(MeasurementValueSourceBusinessDelegate.class.getName());
}
