 ...
 *   <li>Improving the available of DCLine related services in the case of a DCLine business related
 *       service failing.
 *   <li>Exposes a simpler, uniform DCLine interface to the business tier, making it easy for
 *       clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill DCLine business related
 *       services.
 * </ol>
 *
 * <p>
 ...
 */
public class DCLineBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createDCLine(CreateDCLineCommand command)
    ...
    try {
      ...
      // --------------------------------------
      if (command.getDCLineId() == null) command.setDCLineId(UUID.randomUUID());
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateDCLineCommand of DCLine is " + command);
  ...
  public DCLine getDCLine(DCLineFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<DCLine> futureEntity =
          queryGateway.query(
              new FindDCLineQuery(new LoadDCLineFilter(summary.getDCLineId())),
   ...
   */
  public List<DCLine> getAllDCLine() throws ProcessingException {
    List<DCLine> list = null;

    try {
      CompletableFuture<List<DCLine>> futureList =
          queryGateway.query(
              new FindAllDCLineQuery(), ResponseTypes.multipleInstancesOf(DCLine.class));
  ...
  /**
   * add DCLine to DCLines
   *
   * @param command AssignDCLinesToDCLineCommand
   * @exception ProcessingException
   */
  public void addToDCLines(AssignDCLinesToDCLineCommand command) throws ProcessingException {

    // -------------------------------------------
    // load the parent
    // -------------------------------------------
    load(command.getDCLineId());

    com.occulue.europeanstandards.commongridmodelexchangestandard.equipmentprofile.dc.delegate
            .DCLineBusinessDelegate
        childDelegate =
            com.occulue.europeanstandards.commongridmodelexchangestandard.equipmentprofile.dc
                .delegate.DCLineBusinessDelegate.getDCLineInstance();
    DCLineBusinessDelegate parentDelegate = DCLineBusinessDelegate.getDCLineInstance();
    UUID childId = command.getAddTo().getDCLineId();
    ...
    try {
      ...
      final String msg = "Failed to add a DCLine as DCLines to DCLine";
  ...
  /**
   * remove DCLine from DCLines
   *
   * @param command RemoveDCLinesFromDCLineCommand
   * @exception ProcessingException
   */
  public void removeFromDCLines(RemoveDCLinesFromDCLineCommand command) throws ProcessingException {
    ...
    com.occulue.europeanstandards.commongridmodelexchangestandard.equipmentprofile.dc.delegate
        ...
        childDelegate =
            com.occulue.europeanstandards.commongridmodelexchangestandard.equipmentprofile.dc
                .delegate.DCLineBusinessDelegate.getDCLineInstance();
    UUID childId = command.getRemoveFrom().getDCLineId();
   ...
   * @return DCLine
   */
  private DCLine load(UUID id) throws ProcessingException {
