...
import com.occulue.europeanstandards.commongridmodelexchangestandard.equipmentprofile.equivalents.validator.*;
 ...
 *   <li>Improving the available of EquivalentNetwork related services in the case of a
 *       EquivalentNetwork business related service failing.
 *   <li>Exposes a simpler, uniform EquivalentNetwork interface to the business tier, making it easy
 *       for clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill EquivalentNetwork business
 *       related services.
 * </ol>
 *
 * <p>
 ...
 */
public class EquivalentNetworkBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createEquivalentNetwork(CreateEquivalentNetworkCommand command)
    ...
    try {
      ...
      // ---------------------------------------
      // issue the CreateEquivalentNetworkCommand - by convention the future return value for a
      // create command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateEquivalentNetworkCommand of EquivalentNetwork is "
              + command);
  ...
  public CompletableFuture<Void> delete(DeleteEquivalentNetworkCommand command)
    ...
    try {
      ...
      final String errMsg =
          "Unable to delete EquivalentNetwork using Id = " + command.getEquivalentNetworkId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
  ...
  public EquivalentNetwork getEquivalentNetwork(EquivalentNetworkFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<EquivalentNetwork> futureEntity =
          queryGateway.query(
              new FindEquivalentNetworkQuery(
                  new LoadEquivalentNetworkFilter(summary.getEquivalentNetworkId())),
              ResponseTypes.instanceOf(EquivalentNetwork.class));

      entity = futureEntity.get();
    } catch (Exception exc) {
      final String errMsg =
          "Unable to locate EquivalentNetwork with id " + summary.getEquivalentNetworkId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
   ...
   */
  public List<EquivalentNetwork> getAllEquivalentNetwork() throws ProcessingException {
    List<EquivalentNetwork> list = null;

    try {
      CompletableFuture<List<EquivalentNetwork>> futureList =
          queryGateway.query(
   ...
   */
  private EquivalentNetwork load(UUID id) throws ProcessingException {
    equivalentNetwork =
        EquivalentNetworkBusinessDelegate.getEquivalentNetworkInstance()
            .getEquivalentNetwork(new EquivalentNetworkFetchOneSummary(id));
    return equivalentNetwork;
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(EquivalentNetworkBusinessDelegate.class.getName());
}
