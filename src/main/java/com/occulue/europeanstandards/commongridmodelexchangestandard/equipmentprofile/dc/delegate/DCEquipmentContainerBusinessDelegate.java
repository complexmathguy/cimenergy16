 ...
 *   <li>Improving the available of DCEquipmentContainer related services in the case of a
 *       DCEquipmentContainer business related service failing.
 *   <li>Exposes a simpler, uniform DCEquipmentContainer interface to the business tier, making it
 *       easy for clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill DCEquipmentContainer
 ...
 */
public class DCEquipmentContainerBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
   ...
   */
  public CompletableFuture<UUID> createDCEquipmentContainer(
      CreateDCEquipmentContainerCommand command)
    ...
    try {
      ...
      // ---------------------------------------
      // issue the CreateDCEquipmentContainerCommand - by convention the future return value for a
      // create command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateDCEquipmentContainerCommand of DCEquipmentContainer is "
              + command);
   ...
   * @exception IllegalArgumentException
   */
  public CompletableFuture<Void> updateDCEquipmentContainer(
      UpdateDCEquipmentContainerCommand command)
      throws ProcessingException, IllegalArgumentException {
  ...
  public CompletableFuture<Void> delete(DeleteDCEquipmentContainerCommand command)
    ...
    try {
      ...
      final String errMsg =
          "Unable to delete DCEquipmentContainer using Id = " + command.getDCEquipmentContainerId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
  ...
  public DCEquipmentContainer getDCEquipmentContainer(DCEquipmentContainerFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<DCEquipmentContainer> futureEntity =
          queryGateway.query(
              new FindDCEquipmentContainerQuery(
                  new LoadDCEquipmentContainerFilter(summary.getDCEquipmentContainerId())),
              ResponseTypes.instanceOf(DCEquipmentContainer.class));

      entity = futureEntity.get();
    } catch (Exception exc) {
      final String errMsg =
          "Unable to locate DCEquipmentContainer with id " + summary.getDCEquipmentContainerId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
   ...
   */
  public List<DCEquipmentContainer> getAllDCEquipmentContainer() throws ProcessingException {
    List<DCEquipmentContainer> list = null;

    try {
      CompletableFuture<List<DCEquipmentContainer>> futureList =
          queryGateway.query(
   ...
   */
  private DCEquipmentContainer load(UUID id) throws ProcessingException {
    dCEquipmentContainer =
        DCEquipmentContainerBusinessDelegate.getDCEquipmentContainerInstance()
            .getDCEquipmentContainer(new DCEquipmentContainerFetchOneSummary(id));
    return dCEquipmentContainer;
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(DCEquipmentContainerBusinessDelegate.class.getName());
}
