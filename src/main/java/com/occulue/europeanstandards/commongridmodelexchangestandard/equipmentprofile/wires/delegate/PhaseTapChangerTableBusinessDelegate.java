 ...
 *   <li>Improving the available of PhaseTapChangerTable related services in the case of a
 *       PhaseTapChangerTable business related service failing.
 *   <li>Exposes a simpler, uniform PhaseTapChangerTable interface to the business tier, making it
 *       easy for clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill PhaseTapChangerTable
 ...
 */
public class PhaseTapChangerTableBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
   ...
   */
  public CompletableFuture<UUID> createPhaseTapChangerTable(
      CreatePhaseTapChangerTableCommand command)
    ...
    try {
      ...
      // ---------------------------------------
      // issue the CreatePhaseTapChangerTableCommand - by convention the future return value for a
      // create command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreatePhaseTapChangerTableCommand of PhaseTapChangerTable is "
              + command);
   ...
   * @exception IllegalArgumentException
   */
  public CompletableFuture<Void> updatePhaseTapChangerTable(
      UpdatePhaseTapChangerTableCommand command)
      throws ProcessingException, IllegalArgumentException {
  ...
  public CompletableFuture<Void> delete(DeletePhaseTapChangerTableCommand command)
    ...
    try {
      ...
      final String errMsg =
          "Unable to delete PhaseTapChangerTable using Id = " + command.getPhaseTapChangerTableId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
  ...
  public PhaseTapChangerTable getPhaseTapChangerTable(PhaseTapChangerTableFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<PhaseTapChangerTable> futureEntity =
          queryGateway.query(
              new FindPhaseTapChangerTableQuery(
                  new LoadPhaseTapChangerTableFilter(summary.getPhaseTapChangerTableId())),
              ResponseTypes.instanceOf(PhaseTapChangerTable.class));

      entity = futureEntity.get();
    } catch (Exception exc) {
      final String errMsg =
          "Unable to locate PhaseTapChangerTable with id " + summary.getPhaseTapChangerTableId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
   ...
   */
  public List<PhaseTapChangerTable> getAllPhaseTapChangerTable() throws ProcessingException {
    List<PhaseTapChangerTable> list = null;

    try {
      CompletableFuture<List<PhaseTapChangerTable>> futureList =
          queryGateway.query(
   ...
   */
  private PhaseTapChangerTable load(UUID id) throws ProcessingException {
    phaseTapChangerTable =
        PhaseTapChangerTableBusinessDelegate.getPhaseTapChangerTableInstance()
            .getPhaseTapChangerTable(new PhaseTapChangerTableFetchOneSummary(id));
    return phaseTapChangerTable;
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(PhaseTapChangerTableBusinessDelegate.class.getName());
}
