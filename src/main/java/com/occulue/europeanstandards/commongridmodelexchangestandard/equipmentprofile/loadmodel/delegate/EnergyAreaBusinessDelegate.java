 ...
 *   <li>Improving the available of EnergyArea related services in the case of a EnergyArea business
 *       related service failing.
 *   <li>Exposes a simpler, uniform EnergyArea interface to the business tier, making it easy for
 *       clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill EnergyArea business
 *       related services.
 * </ol>
 *
 * <p>
 ...
 */
public class EnergyAreaBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createEnergyArea(CreateEnergyAreaCommand command)
    ...
    try {
      ...
      // --------------------------------------
      if (command.getEnergyAreaId() == null) command.setEnergyAreaId(UUID.randomUUID());
      ...
      // ---------------------------------------
      // issue the CreateEnergyAreaCommand - by convention the future return value for a create
      // command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateEnergyAreaCommand of EnergyArea is " + command);
  ...
  public EnergyArea getEnergyArea(EnergyAreaFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<EnergyArea> futureEntity =
          queryGateway.query(
              new FindEnergyAreaQuery(new LoadEnergyAreaFilter(summary.getEnergyAreaId())),
   ...
   */
  public List<EnergyArea> getAllEnergyArea() throws ProcessingException {
    List<EnergyArea> list = null;

    try {
      CompletableFuture<List<EnergyArea>> futureList =
          queryGateway.query(
              new FindAllEnergyAreaQuery(), ResponseTypes.multipleInstancesOf(EnergyArea.class));
   ...
   * @return EnergyArea
   */
  private EnergyArea load(UUID id) throws ProcessingException {
    energyArea =
        EnergyAreaBusinessDelegate.getEnergyAreaInstance()
            .getEnergyArea(new EnergyAreaFetchOneSummary(id));
    return energyArea;
  }
