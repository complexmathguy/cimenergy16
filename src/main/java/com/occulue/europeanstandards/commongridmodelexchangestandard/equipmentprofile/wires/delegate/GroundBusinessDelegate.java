 ...
 *   <li>Improving the available of Ground related services in the case of a Ground business related
 *       service failing.
 *   <li>Exposes a simpler, uniform Ground interface to the business tier, making it easy for
 *       clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill Ground business related
 *       services.
 * </ol>
 *
 * <p>
 ...
 */
public class GroundBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createGround(CreateGroundCommand command)
    ...
    try {
      ...
      // --------------------------------------
      if (command.getGroundId() == null) command.setGroundId(UUID.randomUUID());
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateGroundCommand of Ground is " + command);
  ...
  public Ground getGround(GroundFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<Ground> futureEntity =
          queryGateway.query(
              new FindGroundQuery(new LoadGroundFilter(summary.getGroundId())),
   ...
   */
  public List<Ground> getAllGround() throws ProcessingException {
    List<Ground> list = null;

    try {
      CompletableFuture<List<Ground>> futureList =
          queryGateway.query(
              new FindAllGroundQuery(), ResponseTypes.multipleInstancesOf(Ground.class));
   ...
   * @return Ground
   */
  private Ground load(UUID id) throws ProcessingException {
