 ...
 *   <li>Improving the available of WindGeneratingUnit related services in the case of a
 *       WindGeneratingUnit business related service failing.
 *   <li>Exposes a simpler, uniform WindGeneratingUnit interface to the business tier, making it
 *       easy for clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill WindGeneratingUnit
 ...
 */
public class WindGeneratingUnitBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createWindGeneratingUnit(CreateWindGeneratingUnitCommand command)
    ...
    try {
      ...
      // ---------------------------------------
      // issue the CreateWindGeneratingUnitCommand - by convention the future return value for a
      // create command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateWindGeneratingUnitCommand of WindGeneratingUnit is "
              + command);
  ...
  public CompletableFuture<Void> delete(DeleteWindGeneratingUnitCommand command)
    ...
    try {
      ...
      final String errMsg =
          "Unable to delete WindGeneratingUnit using Id = " + command.getWindGeneratingUnitId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
  ...
  public WindGeneratingUnit getWindGeneratingUnit(WindGeneratingUnitFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<WindGeneratingUnit> futureEntity =
          queryGateway.query(
              new FindWindGeneratingUnitQuery(
                  new LoadWindGeneratingUnitFilter(summary.getWindGeneratingUnitId())),
              ResponseTypes.instanceOf(WindGeneratingUnit.class));

      entity = futureEntity.get();
    } catch (Exception exc) {
      final String errMsg =
          "Unable to locate WindGeneratingUnit with id " + summary.getWindGeneratingUnitId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
   ...
   */
  public List<WindGeneratingUnit> getAllWindGeneratingUnit() throws ProcessingException {
    List<WindGeneratingUnit> list = null;

    try {
      CompletableFuture<List<WindGeneratingUnit>> futureList =
          queryGateway.query(
   ...
   */
  private WindGeneratingUnit load(UUID id) throws ProcessingException {
    windGeneratingUnit =
        WindGeneratingUnitBusinessDelegate.getWindGeneratingUnitInstance()
            .getWindGeneratingUnit(new WindGeneratingUnitFetchOneSummary(id));
    return windGeneratingUnit;
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(WindGeneratingUnitBusinessDelegate.class.getName());
}
