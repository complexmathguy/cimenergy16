...
import com.occulue.europeanstandards.commongridmodelexchangestandard.equipmentprofile.core.validator.*;
 ...
 *   <li>Improving the available of Substation related services in the case of a Substation business
 *       related service failing.
 *   <li>Exposes a simpler, uniform Substation interface to the business tier, making it easy for
 *       clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill Substation business
 *       related services.
 * </ol>
 *
 * <p>
 ...
 */
public class SubstationBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createSubstation(CreateSubstationCommand command)
    ...
    try {
      ...
      // --------------------------------------
      if (command.getSubstationId() == null) command.setSubstationId(UUID.randomUUID());
      ...
      // ---------------------------------------
      // issue the CreateSubstationCommand - by convention the future return value for a create
      // command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateSubstationCommand of Substation is " + command);
  ...
  public Substation getSubstation(SubstationFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<Substation> futureEntity =
          queryGateway.query(
              new FindSubstationQuery(new LoadSubstationFilter(summary.getSubstationId())),
   ...
   */
  public List<Substation> getAllSubstation() throws ProcessingException {
    List<Substation> list = null;

    try {
      CompletableFuture<List<Substation>> futureList =
          queryGateway.query(
              new FindAllSubstationQuery(), ResponseTypes.multipleInstancesOf(Substation.class));
  ...
  /**
   * add Substation to Substations
   *
   * @param command AssignSubstationsToSubstationCommand
   * @exception ProcessingException
   */
  public void addToSubstations(AssignSubstationsToSubstationCommand command)
      throws ProcessingException {

    // -------------------------------------------
    // load the parent
    // -------------------------------------------
    load(command.getSubstationId());

    com.occulue.europeanstandards.commongridmodelexchangestandard.equipmentprofile.core.delegate
            .SubstationBusinessDelegate
        childDelegate =
            com.occulue.europeanstandards.commongridmodelexchangestandard.equipmentprofile.core
                .delegate.SubstationBusinessDelegate.getSubstationInstance();
    SubstationBusinessDelegate parentDelegate = SubstationBusinessDelegate.getSubstationInstance();
    UUID childId = command.getAddTo().getSubstationId();
    ...
    try {
      ...
      final String msg = "Failed to add a Substation as Substations to Substation";
  ...
  /**
   * remove Substation from Substations
   *
   * @param command RemoveSubstationsFromSubstationCommand
   * @exception ProcessingException
   */
  public void removeFromSubstations(RemoveSubstationsFromSubstationCommand command)
      throws ProcessingException {
    ...
    com.occulue.europeanstandards.commongridmodelexchangestandard.equipmentprofile.core.delegate
        ...
        childDelegate =
            com.occulue.europeanstandards.commongridmodelexchangestandard.equipmentprofile.core
                .delegate.SubstationBusinessDelegate.getSubstationInstance();
    UUID childId = command.getRemoveFrom().getSubstationId();
   ...
   * @return Substation
   */
  private Substation load(UUID id) throws ProcessingException {
    substation =
        SubstationBusinessDelegate.getSubstationInstance()
            .getSubstation(new SubstationFetchOneSummary(id));
    return substation;
  }
