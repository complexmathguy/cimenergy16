 ...
 *   <li>Improving the available of Breaker related services in the case of a Breaker business
 *       related service failing.
 *   <li>Exposes a simpler, uniform Breaker interface to the business tier, making it easy for
 *       clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill Breaker business related
 *       services.
 * </ol>
 *
 * <p>
 ...
 */
public class BreakerBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createBreaker(CreateBreakerCommand command)
    ...
    try {
      ...
      // --------------------------------------
      if (command.getBreakerId() == null) command.setBreakerId(UUID.randomUUID());
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateBreakerCommand of Breaker is " + command);
  ...
  public Breaker getBreaker(BreakerFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<Breaker> futureEntity =
          queryGateway.query(
              new FindBreakerQuery(new LoadBreakerFilter(summary.getBreakerId())),
   ...
   */
  public List<Breaker> getAllBreaker() throws ProcessingException {
    List<Breaker> list = null;

    try {
      CompletableFuture<List<Breaker>> futureList =
          queryGateway.query(
              new FindAllBreakerQuery(), ResponseTypes.multipleInstancesOf(Breaker.class));
   ...
   * @return Breaker
   */
  private Breaker load(UUID id) throws ProcessingException {
    breaker =
        BreakerBusinessDelegate.getBreakerInstance().getBreaker(new BreakerFetchOneSummary(id));
    return breaker;
  }
