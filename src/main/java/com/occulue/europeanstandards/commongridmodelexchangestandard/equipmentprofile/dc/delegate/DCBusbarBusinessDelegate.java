 ...
 *   <li>Improving the available of DCBusbar related services in the case of a DCBusbar business
 *       related service failing.
 *   <li>Exposes a simpler, uniform DCBusbar interface to the business tier, making it easy for
 *       clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill DCBusbar business related
 *       services.
 * </ol>
 *
 * <p>
 ...
 */
public class DCBusbarBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createDCBusbar(CreateDCBusbarCommand command)
    ...
    try {
      ...
      // --------------------------------------
      if (command.getDCBusbarId() == null) command.setDCBusbarId(UUID.randomUUID());
      ...
      // ---------------------------------------
      // issue the CreateDCBusbarCommand - by convention the future return value for a create
      // command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateDCBusbarCommand of DCBusbar is " + command);
  ...
  public DCBusbar getDCBusbar(DCBusbarFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<DCBusbar> futureEntity =
          queryGateway.query(
              new FindDCBusbarQuery(new LoadDCBusbarFilter(summary.getDCBusbarId())),
   ...
   */
  public List<DCBusbar> getAllDCBusbar() throws ProcessingException {
    List<DCBusbar> list = null;

    try {
      CompletableFuture<List<DCBusbar>> futureList =
          queryGateway.query(
              new FindAllDCBusbarQuery(), ResponseTypes.multipleInstancesOf(DCBusbar.class));
   ...
   * @return DCBusbar
   */
  private DCBusbar load(UUID id) throws ProcessingException {
    dCBusbar =
        DCBusbarBusinessDelegate.getDCBusbarInstance().getDCBusbar(new DCBusbarFetchOneSummary(id));
    return dCBusbar;
  }
