 ...
 *   <li>Improving the available of LoadArea related services in the case of a LoadArea business
 *       related service failing.
 *   <li>Exposes a simpler, uniform LoadArea interface to the business tier, making it easy for
 *       clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill LoadArea business related
 *       services.
 * </ol>
 *
 * <p>
 ...
 */
public class LoadAreaBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createLoadArea(CreateLoadAreaCommand command)
    ...
    try {
      ...
      // --------------------------------------
      if (command.getLoadAreaId() == null) command.setLoadAreaId(UUID.randomUUID());
      ...
      // ---------------------------------------
      // issue the CreateLoadAreaCommand - by convention the future return value for a create
      // command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateLoadAreaCommand of LoadArea is " + command);
  ...
  public LoadArea getLoadArea(LoadAreaFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<LoadArea> futureEntity =
          queryGateway.query(
              new FindLoadAreaQuery(new LoadLoadAreaFilter(summary.getLoadAreaId())),
   ...
   */
  public List<LoadArea> getAllLoadArea() throws ProcessingException {
    List<LoadArea> list = null;

    try {
      CompletableFuture<List<LoadArea>> futureList =
          queryGateway.query(
              new FindAllLoadAreaQuery(), ResponseTypes.multipleInstancesOf(LoadArea.class));
   ...
   * @return LoadArea
   */
  private LoadArea load(UUID id) throws ProcessingException {
    loadArea =
        LoadAreaBusinessDelegate.getLoadAreaInstance().getLoadArea(new LoadAreaFetchOneSummary(id));
    return loadArea;
  }
