 ...
 *   <li>Improving the available of HydroPowerPlant related services in the case of a
 *       HydroPowerPlant business related service failing.
 *   <li>Exposes a simpler, uniform HydroPowerPlant interface to the business tier, making it easy
 *       for clients to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill HydroPowerPlant business
 *       related services.
 * </ol>
 *
 * <p>
 ...
 */
public class HydroPowerPlantBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createHydroPowerPlant(CreateHydroPowerPlantCommand command)
    ...
    try {
      ...
      // --------------------------------------
      if (command.getHydroPowerPlantId() == null) command.setHydroPowerPlantId(UUID.randomUUID());
      ...
      // ---------------------------------------
      // issue the CreateHydroPowerPlantCommand - by convention the future return value for a create
      // command
      ...
      LOGGER.log(
          Level.INFO,
          "return from Command Gateway for CreateHydroPowerPlantCommand of HydroPowerPlant is "
              + command);
  ...
  public CompletableFuture<Void> delete(DeleteHydroPowerPlantCommand command)
    ...
    try {
      ...
      final String errMsg =
          "Unable to delete HydroPowerPlant using Id = " + command.getHydroPowerPlantId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
  ...
  public HydroPowerPlant getHydroPowerPlant(HydroPowerPlantFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<HydroPowerPlant> futureEntity =
          queryGateway.query(
              new FindHydroPowerPlantQuery(
                  new LoadHydroPowerPlantFilter(summary.getHydroPowerPlantId())),
              ResponseTypes.instanceOf(HydroPowerPlant.class));

      entity = futureEntity.get();
    } catch (Exception exc) {
      final String errMsg =
          "Unable to locate HydroPowerPlant with id " + summary.getHydroPowerPlantId();
      LOGGER.log(Level.WARNING, errMsg, exc);
      throw new ProcessingException(errMsg, exc);
    } finally {
    }
   ...
   */
  public List<HydroPowerPlant> getAllHydroPowerPlant() throws ProcessingException {
    List<HydroPowerPlant> list = null;

    try {
      CompletableFuture<List<HydroPowerPlant>> futureList =
          queryGateway.query(
   ...
   */
  private HydroPowerPlant load(UUID id) throws ProcessingException {
    hydroPowerPlant =
        HydroPowerPlantBusinessDelegate.getHydroPowerPlantInstance()
            .getHydroPowerPlant(new HydroPowerPlantFetchOneSummary(id));
    return hydroPowerPlant;
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(HydroPowerPlantBusinessDelegate.class.getName());
}
