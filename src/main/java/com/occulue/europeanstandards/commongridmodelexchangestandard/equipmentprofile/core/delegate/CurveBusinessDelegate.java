 ...
 *   <li>Improving the available of Curve related services in the case of a Curve business related
 *       service failing.
 *   <li>Exposes a simpler, uniform Curve interface to the business tier, making it easy for clients
 *       to consume a simple Java object.
 *   <li>Hides the communication protocol that may be required to fulfill Curve business related
 *       services.
 * </ol>
 *
 * <p>
 ...
 */
public class CurveBusinessDelegate extends BaseBusinessDelegate {
  // ************************************************************************
  // Public Methods
  // ************************************************************************
  /** Default Constructor */
  ...
  public CompletableFuture<UUID> createCurve(CreateCurveCommand command)
    ...
    try {
      ...
      // --------------------------------------
      if (command.getCurveId() == null) command.setCurveId(UUID.randomUUID());
      ...
      LOGGER.log(
          Level.INFO, "return from Command Gateway for CreateCurveCommand of Curve is " + command);
  ...
  public Curve getCurve(CurveFetchOneSummary summary)
    ...
    try {
      ...
      // --------------------------------------
      CompletableFuture<Curve> futureEntity =
          queryGateway.query(
              new FindCurveQuery(new LoadCurveFilter(summary.getCurveId())),
              ResponseTypes.instanceOf(Curve.class));
   ...
   */
  public List<Curve> getAllCurve() throws ProcessingException {
    List<Curve> list = null;

    try {
      CompletableFuture<List<Curve>> futureList =
          queryGateway.query(
              new FindAllCurveQuery(), ResponseTypes.multipleInstancesOf(Curve.class));
   ...
   * @return Curve
   */
  private Curve load(UUID id) throws ProcessingException {
